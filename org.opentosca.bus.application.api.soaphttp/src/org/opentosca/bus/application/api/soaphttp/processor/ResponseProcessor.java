...
public class ResponseProcessor implements Processor {
	...
	public void process(Exchange exchange) throws Exception {
		...
		if (exchange.getIn().getBody() instanceof Exception) {
			...
			Exception exception = exchange.getIn().getBody(Exception.class);

			org.opentosca.bus.application.api.soaphttp.model.ApplicationBusException e = new org.opentosca.bus.application.api.soaphttp.model.ApplicationBusException();
			e.setMessage(exception.getMessage());

			JAXBElement<org.opentosca.bus.application.api.soaphttp.model.ApplicationBusException> jaxbElement = objectFactory
					.createApplicationBusException(e);
		...
		if (operation.equals("invokeMethodWithServiceInstanceID")) {
			...
			InvokeMethodWithServiceInstanceIDResponse invokeResponse = new InvokeMethodWithServiceInstanceIDResponse();
			invokeResponse.setRequestID(exchange.getIn().getBody(String.class));

			JAXBElement<InvokeMethodWithServiceInstanceIDResponse> jaxbElement = objectFactory
					.createInvokeMethodWithServiceInstanceIDResponse(invokeResponse);
		...
		if (operation.equals("invokeMethodWithNodeInstanceID")) {
			...
			InvokeMethodWithNodeInstanceIDResponse invokeResponse = new InvokeMethodWithNodeInstanceIDResponse();
			invokeResponse.setRequestID(exchange.getIn().getBody(String.class));

			JAXBElement<InvokeMethodWithNodeInstanceIDResponse> jaxbElement = objectFactory
					.createInvokeMethodWithNodeInstanceIDResponse(invokeResponse);
		...
		if (operation.equals("isFinished")) {
			...
			IsFinishedResponse isFinishedResponse = new IsFinishedResponse();
			isFinishedResponse.setIsFinished(exchange.getIn().getBody(Boolean.class));

			JAXBElement<IsFinishedResponse> jaxbElement = objectFactory.createIsFinishedResponse(isFinishedResponse);
		...
		if (operation.equals("getResult")) {
			...
			GetResultResponse resultResponse = new GetResultResponse();
			resultResponse.setResult(exchange.getIn().getBody());

			JAXBElement<GetResultResponse> jaxbElement = objectFactory.createGetResultResponse(resultResponse);
