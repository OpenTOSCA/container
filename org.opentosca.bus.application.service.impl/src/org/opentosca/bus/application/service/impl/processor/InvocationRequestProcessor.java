...
public class InvocationRequestProcessor implements Processor {
	...
	public void process(final Exchange exchange) throws Exception {
		...
		final Integer serviceInstanceID = message.getHeader(ApplicationBusConstants.SERVICE_INSTANCE_ID_INT.toString(), Integer.class);
		...
		final Integer nodeInstanceID = message.getHeader(ApplicationBusConstants.NODE_INSTANCE_ID_INT.toString(), Integer.class);
		...
		final NodeInstance nodeInstance = ContainerProxy.getNodeInstance(serviceInstanceID, nodeInstanceID, nodeTemplateID);
		...
		if (nodeInstance != null) {
			...
			InvocationRequestProcessor.LOG.debug("Matching NodeInstance found: ID: " + nodeInstance.getNodeInstanceID() + " CSAR-ID: " + csarID + " ServiceTemplateID: " + serviceTemplateID + " NodeTemplateID: " + nodeTemplateID + " of type: " + nodeType);
					...
					if (hostedOnNodeTemplateID != null) {
						...
						if (hostedOnNodeURL != null) {
							...
							try {
								...
								InvocationRequestProcessor.LOG.error("Generating endpoint for Node: {} failed!", nodeTemplateID);
								e.printStackTrace();
							}
						}
					}
		...
		if (endpoint != null) {
			...
			InvocationRequestProcessor.LOG.debug("Searching an Application Bus Plugin for InvocationType: {}", invocationType);
			...
			if (appBusPluginEndpoint != null) {
				...
				InvocationRequestProcessor.LOG.debug("Application Bus Plugin with matching InvocationType: {} found. Endpoint: {}", invocationType, appBusPluginEndpoint);
