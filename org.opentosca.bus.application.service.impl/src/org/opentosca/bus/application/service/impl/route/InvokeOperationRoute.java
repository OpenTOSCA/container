...
public class InvokeOperationRoute extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		ParameterCheckProcessor checkProcessor = new ParameterCheckProcessor();
		InvocationRequestProcessor requestProcessor = new InvocationRequestProcessor();
		...
		from(MainRoute.INVOKE_ENDPOINT).doTry().process(checkProcessor).doCatch(ApplicationBusExternalException.class)
				...
				.setHeader(APPLICATION_BUS_REQUEST_ID_HEADER, method(RequestID.class, "getNextID"))
		...
		from("direct:invokeProcess").setExchangePattern(ExchangePattern.InOut).process(requestProcessor).choice()
				.when(header(APPLICATION_BUS_PLUGIN_ENDPOINT_HEADER).isNotNull()).to("direct:toPlugin").endChoice()
				.otherwise()
		...
		from("direct:handleResponse")
				.bean(QueueMap.class, "finished(${header." + APPLICATION_BUS_REQUEST_ID_HEADER + "})")
				.bean(ResultMap.class, "put(${header." + APPLICATION_BUS_REQUEST_ID_HEADER + "}, ${body})").stop();
