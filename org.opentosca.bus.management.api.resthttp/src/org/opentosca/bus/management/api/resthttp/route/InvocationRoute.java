...
public class InvocationRoute extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		final Predicate INVOKE_IA = PredicateBuilder.or(this.header(MBHeader.NODETEMPLATEID_STRING.toString()).isNotNull(), this.header(MBHeader.PLANID_QNAME.toString()).isNotNull());
		...
		// handle exceptions
		this.onException(Exception.class).handled(true).setBody(this.property(Exchange.EXCEPTION_CAUGHT)).process(corsProcessor).process(exceptionProcessor);
		
		// invoke main route
		this.from("restlet:" + BASE_ENDPOINT + INVOKE_ENDPOINT + "?restletMethods=post").doTry().process(invocationRequestProcessor).doCatch(Exception.class).end().choice().when(this.property(Exchange.EXCEPTION_CAUGHT).isNull()).to("direct:invoke").otherwise().to("direct:exception").endChoice().removeHeaders("*");
		
		// route if no exception was caught
		this.from("direct:invoke").setHeader(MANAGEMENT_BUS_REQUEST_ID_HEADER, this.method(RequestID.class, "getNextID")).wireTap("direct:toManagementBus").end().to("direct:init").process(corsProcessor).process(invocationResponseProcessor);
		...
		this.from("direct:init").bean(QueueMap.class, "notFinished(${header." + MANAGEMENT_BUS_REQUEST_ID_HEADER + "})").setBody(this.simple("${header." + MANAGEMENT_BUS_REQUEST_ID_HEADER + "}"));
		
		// route to management bus engine
		this.from("direct:toManagementBus").choice().when(INVOKE_IA).to(MANAGEMENT_BUS_IA).when(INVOKE_PLAN).to(MANAGEMENT_BUS_PLAN).end();
		
		// invoke response route
		this.from("direct-vm:" + Activator.apiID).bean(QueueMap.class, "finished(${header." + MANAGEMENT_BUS_REQUEST_ID_HEADER + "})").bean(ResultMap.class, "put(${header." + MANAGEMENT_BUS_REQUEST_ID_HEADER + "}, ${body})").stop();
