...
public class InvocationRoute extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		// invoke main route
		this.from("restlet:" + BASE_ENDPOINT + INVOKE_ENDPOINT + "?restletMethods=post").doTry().process(invocationRequestProcessor).doCatch(Exception.class).end().choice().when(this.property(Exchange.EXCEPTION_CAUGHT).isNull()).to("direct:invoke").otherwise().to("direct:exception").end().removeHeaders("*");
