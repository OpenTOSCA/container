...
public class Route extends RouteBuilder {
  ...
  public final static String ENDPOINT =
      "http://" + Settings.OPENTOSCA_CONTAINER_HOSTNAME + ":8081/invoker";
  ...
  public void configure() throws Exception {
    ...
    final ClassLoader cl =
        org.opentosca.bus.management.api.soaphttp.model.ObjectFactory.class.getClassLoader();
    final JAXBContext jc =
        JAXBContext.newInstance("org.opentosca.bus.management.api.soaphttp.model", cl);
    ...
    this.from(INVOKE_ENDPOINT).unmarshal(requestJaxb).process(requestProcessor).choice()
        .when(INVOKE_IA).to(MANAGEMENT_BUS_IA).when(INVOKE_PLAN).to(MANAGEMENT_BUS_PLAN).end();
    this.from("direct-vm:" + Activator.apiID).process(responseProcessor).marshal(responseJaxb)
        .to("stream:out").choice().when(ASYNC).recipientList(this.simple(CALLBACK_ENDPOINT)).end();
  }
}
