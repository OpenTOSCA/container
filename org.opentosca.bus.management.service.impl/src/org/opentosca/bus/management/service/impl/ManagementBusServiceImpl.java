...
public class ManagementBusServiceImpl implements IManagementBusService {
	...
	public void invokeIA(Exchange exchange) {
		...
		if (nodeTemplateID != null) {
			...
			if (message.getBody() instanceof HashMap) {
				...
				inputParams = updateInputParams(inputParams, csarID, serviceTemplateID, nodeTypeID, nodeTemplateID, neededInterface, neededOperation, serviceInstanceID);
			...
			searchIA: for (QName nodeTypeImplementationID : nodeTypeImplementationIDs) {
				...
				for (String implementationArtifactName : implementationArtifactNames) {
					...
					if (isCorrectIA(csarID, nodeTypeID, nodeTypeImplementationID, null, null, implementationArtifactName, neededOperation, neededInterface)) {
						...
						if (invocationType != null) {
							...
							if (wsdlEndpoint != null) {
								...
								if (endpoint.toString().startsWith("/PLACEHOLDER_") && endpoint.toString().contains("_PLACEHOLDER/")) {
									...
									endpoint = replacePlaceholderWithInstanceData(endpoint, csarID, serviceTemplateID, nodeTemplateID, serviceInstanceID);
								}
			...
			searchIA: for (QName relationshipTypeImplementationID : relationshipTypeImplementationIDs) {
				...
				for (String implementationArtifactName : implementationArtifactNames) {
					...
					// Check if needed interface/operation is provided
					if (isCorrectIA(csarID, null, null, relationshipTypeID, relationshipTypeImplementationID, implementationArtifactName, neededOperation, neededInterface)) {
	...
	private String isSupported(String artifactType, CSARID csarID, QName artifactTemplateID) {
		...
		if (ServiceHandler.pluginServices.containsKey(artifactType)) {
			...
			// TOSCA definition
			String invocationType = getInvocationType(properties);
	 ...
	 */
	private HashMap<String, String> updateInputParams(HashMap<String, String> inputParams, CSARID csarID, QName serviceTemplateID, QName nodeTypeID, String nodeTemplateID, String neededInterface, String neededOperation, URI serviceInstanceID) {
		...
		if (!expectedParams.isEmpty()) {
			...
			if ((serviceInstanceID != null) && (!serviceInstanceID.toString().equals("?"))) {
				...
				if (propertiesMap != null) {
					...
					List<String> supportedInstanceIdPropertyNames = Utils.getSupportedVirtualMachineInstanceIdPropertyNames();
					List<String> supportedPasswordPropertyNames = Utils.getSupportedVirtualMachineLoginPasswordPropertyNames();
					List<String> supportedUsernamePropertyNames = Utils.getSupportedVirtualMachineLoginUserNamePropertyNames();
	 ...
	 */
	private URI replacePlaceholderWithInstanceData(URI endpoint, CSARID csarID, QName serviceTemplateID, String nodeTemplateID, URI serviceInstanceID) {
		
		String placeholderBegin = "/PLACEHOLDER_";
		String placeholderEnd = "_PLACEHOLDER/";
		
		String placeholder = endpoint.toString().substring(endpoint.toString().lastIndexOf(placeholderBegin), endpoint.toString().lastIndexOf(placeholderEnd) + (placeholderEnd).length());
		
		ManagementBusServiceImpl.LOG.debug("Placeholder: {} detected in Endpoint: {}", placeholder, endpoint.toString());
		
		String[] placeholderProperties = placeholder.replace(placeholderBegin, "").replace(placeholderEnd, "").split("_");
