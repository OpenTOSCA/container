 ...
 */
public class AppBusJsonIsFinishedResponseProcessor implements Processor {

    final private static Logger LOG = LoggerFactory.getLogger(AppBusJsonIsFinishedResponseProcessor.class);

    @Override
    public void process(final Exchange exchange) throws Exception {

        AppBusJsonIsFinishedResponseProcessor.LOG.debug("Processing IsFinished response....");

        final String requestID = exchange.getIn().getHeader(Route.ID, String.class);

        AppBusJsonIsFinishedResponseProcessor.LOG.debug("RequestID: {}", requestID);
        ...
        if (exchange.getIn().getBody() instanceof Exception) {
            ...
            if (isFinished) {
                AppBusJsonIsFinishedResponseProcessor.LOG.debug("Invocation has finished, send location of result.");
                ...
                AppBusJsonIsFinishedResponseProcessor.LOG.debug("Invocation has not finished yet.");
