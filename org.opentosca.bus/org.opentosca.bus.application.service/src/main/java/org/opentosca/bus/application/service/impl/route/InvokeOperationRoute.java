...
public class InvokeOperationRoute extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        // handle exceptions
        onException(Exception.class).setBody(exchangeProperty(Exchange.EXCEPTION_CAUGHT)).to("direct:handleResponse");
        ...
        from(MainRoute.INVOKE_ENDPOINT).doTry().process(ParameterCheckProcessor.BEAN_NAME)
            .doCatch(ApplicationBusExternalException.class).end().choice()
        ...
        from("direct:invokeProcess").setExchangePattern(ExchangePattern.InOut)
            .process(InvocationRequestProcessor.BEAN_NAME).choice()
        ...
        from("direct:toPlugin").doTry().recipientList(header(APPLICATION_BUS_PLUGIN_ENDPOINT_HEADER)).end()
            .doCatch(Exception.class).setBody(exchangeProperty(Exchange.EXCEPTION_CAUGHT))
            .doFinally().to("direct:handleResponse").end();
