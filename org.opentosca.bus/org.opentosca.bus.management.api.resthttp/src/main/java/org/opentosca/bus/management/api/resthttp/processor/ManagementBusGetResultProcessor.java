 ...
 */
@Component
public class ManagementBusGetResultProcessor implements Processor {

    final private static Logger LOG = LoggerFactory.getLogger(ManagementBusGetResultProcessor.class);
    ...
    public void process(final Exchange exchange) throws Exception {
        ...
        ManagementBusGetResultProcessor.LOG.debug("getResult request received. RequestID: {}", requestID);

        if (ResultMap.containsID(requestID)) {

            ManagementBusGetResultProcessor.LOG.debug("Getting result...");
            ...
            ManagementBusGetResultProcessor.LOG.warn("Unknown RequestID: {}", requestID);
            exchange.getIn().setBody(new Exception("Unknown RequestID: " + requestID));
        } else {
            ManagementBusGetResultProcessor.LOG.warn("Error while invoking specified method.");
