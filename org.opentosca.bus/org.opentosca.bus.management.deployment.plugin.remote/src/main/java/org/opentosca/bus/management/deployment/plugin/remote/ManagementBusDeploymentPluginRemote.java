 ...
 * <br>
 * <p>
 * This Plug-in is able to deploy and undeploy all kind of IAs which are supported by one of the other available
 * deployment plug-ins on a remote OpenTOSCA Container. It gets a camel exchange object from the Management Bus which
 * contains all information that is needed for the deployment/undeployment. Afterwards it forwards the information via
 * MQTT to the remote Container and waits for a response. When the response arrives the result of the
