...
public class DeploymentDistributionDecisionMaker {
     ...
     * collaboration mode is turned on, this method performs an instance data matching to determine the
     * deployment location. Therefore, the infrastructure NodeTemplateInstance is searched in the
     * topology. Afterwards, its type and properties are matched against local and remote instance data
     * to get the correct deployment location for the IAs. If the matching is not successful, the local
     * OpenTOSCA Container is returned as default deployment location.
     *
     ...
     * hosted/deployed/based. In the context of instance data matching the infrastructure Node should
     * always be the Node at the bottom of a stack in the topology. If an OpenTOSCA Container manages
     * this bottom Node, it can be used to deploy all IAs attached to Nodes that are above the
     ...
     * matching is successful if a NodeTemplateInstance with the same NodeType and the same values for
     * the properties is found in the instance data.
     *
    ...
    /**
     * Match the given NodeType and properties against instance data from remote OpenTOSCA Containers.
     * The matching is successful if a NodeTemplateInstance with the same NodeType and the same values
     * for the properties is found in their instance data. The method sends a request via MQTT to all
     * subscribed OpenTOSCA Containers. Afterwards, it waits for a reply which contains the host name of
     * the OpenTOSCA Container that found matching instance data. If it receives a reply in time, it
     * returns the host name. Otherwise, it returns null.
     *
     * @param infrastructureNodeType the NodeType of the NodeTemplate which has to be matched
     * @param infrastructureProperties the set of properties of the NodeTemplate which has to be matched
     * @return the host name of the OpenTOSCA Container which found a matching NodeTemplateInstance if
     *         one is found, <tt>null</tt> otherwise.
     */
