...
public class ExistenceChecker {
	...
	public static boolean existsServiceInstance(final URI serviceInstanceID, final IInstanceDataService service) {
		final List<ServiceInstance> serviceInstances = service.getServiceInstances(serviceInstanceID, null, null);
	...
	public static boolean existsNodeInstance(final URI nodeInstanceID, final IInstanceDataService service) {
		final List<NodeInstance> nodeInstances = service.getNodeInstances(nodeInstanceID, null, null, null);
		// check if only one instance was returned - we dont verify because we
		// assume the get Method returns the correct one!
		// we got really bad problems if this wouldnt work anyway
	 ...
	 */
	public static NodeInstance checkNodeInstanceWithException(final int nodeInstanceID, final IInstanceDataService service) throws GenericRestException {
		final List<NodeInstance> nodeInstances = service.getNodeInstances(IdConverter.nodeInstanceIDtoURI(nodeInstanceID), null, null, null);
		// check if only one instance was returned - we dont verify because we
		// assume the get Method returns the correct one!
		// we got really bad problems if this wouldnt work anyway
		if ((nodeInstances != null) && (nodeInstances.size() == 1) && (nodeInstances.get(0) != null)) {
	...
	public static ServiceInstance checkServiceInstanceWithException(final int serviceInstanceID, final IInstanceDataService service) throws GenericRestException {
		final List<ServiceInstance> serviceInstances = service.getServiceInstances(IdConverter.serviceInstanceIDtoURI(serviceInstanceID), null, null);
		// check if only one instance was returned - we dont verify because we
		// assume the get Method returns the correct one!
		// we got really bad problems if this wouldnt work anyway
		if ((serviceInstances != null) && (serviceInstances.size() == 1) && (serviceInstances.get(0) != null)) {
