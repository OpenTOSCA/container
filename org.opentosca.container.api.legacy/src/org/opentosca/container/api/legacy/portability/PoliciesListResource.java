...
public class PoliciesListResource {
	...
	public TBoundaryDefinitions.Policies getPolicies(@QueryParam("csarID") final String csarID, @QueryParam("namespace") final String namespaceServiceTemplate, @QueryParam("serviceTemplateID") final String serviceTemplateID, @QueryParam("nodeTemplateID") final String nodeTemplateID) {
		
		if (Utilities.areEmpty(csarID, serviceTemplateID)) {
			throw new GenericRestException(Status.BAD_REQUEST, "one of the required parameters: csarID, serviceTemplateID was not set");
		}
		...
		try {
			...
			throw new GenericRestException(Status.BAD_REQUEST, "error converting one of the parameters: " + e.getMessage());
		}
		...
		Policies policies = ToscaServiceHandler.getToscaEngineService().getToscaReferenceMapper().getPolicies(csarID_csarID, templateID);
