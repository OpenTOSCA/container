package org.opentosca.container.api.legacy.resources.credentials;
...
import org.opentosca.container.api.legacy.resources.credentials.jaxb.CredentialsJaxb;
import org.opentosca.container.api.legacy.resources.credentials.jaxb.JaxbFactory;
...
public class CredentialsResource {
    ...
    /**
     * If {@code null} it's a not existing credentials and every method returns 404 (not found).
     */
     ...
     * @param credentials
     */
    public CredentialsResource(final Credentials credentials) {
        CredentialsResource.LOG.debug("{} created: {}", this.getClass(), this);
        this.CREDENTIALS = credentials;
    }

    /**
     * @return 200 (OK) with entity {@link CredentialsJaxb} containing the credentials to show. If
     *         credentials not exist 404 (not found).
     */
    @GET
    ...
    /**
     * Sets or deletes these credentials in their storage provider if {@code set} respectively
     * {@code unset} is passed in {@code input} (body of a POST message).
     *
     * @param input
     * @return 200 (OK) - credentials were set respectively deleted in their storage provider.<br />
     *         400 (bad request) - credentials are currently not set in their storage provider or
     *         neither {@code set} nor {@code unset} were passed.<br />
     *         404 (not found) - credentials don't exist.<br />
     *         500 (internal server error) - setting / deleting credentials failed.
     * @throws SystemException
     * @throws UserException
     *
     * @see ICoreCredentialsService#setCredentialsInStorageProvider(String, String)
    ...
    @Consumes(MediaType.TEXT_PLAIN)
    @Produces(MediaType.TEXT_PLAIN)
    public Response setOrUnsetCredentialsInStorageProvider(final String input) throws SystemException, UserException {

        if (this.CREDENTIALS == null) {
            return Response.status(Status.NOT_FOUND).build();
        }

        final long credentialsID = this.CREDENTIALS.getID();
        final String storageProviderID = this.CREDENTIALS.getStorageProviderID();
        ...
        if (input.equalsIgnoreCase("set")) {
            ...
            return Response.ok("Credentials \"" + credentialsID + "\" were set in their storage provider \""
                + storageProviderID + "\".").build();

        } else if (input.equalsIgnoreCase("unset")) {

            // we must check at first if storage provider has currently
            // these credentials
            final boolean hasThisCredentials = this.CREDENTIALS_SERVICE.hasStorageProviderCredentials(credentialsID);

            if (hasThisCredentials) {
                this.CREDENTIALS_SERVICE.deleteCredentialsInStorageProvider(storageProviderID);
                return Response.ok("Credentials \"" + credentialsID + "\" were deleted in their storage provider \""
                    + storageProviderID + "\".").build();
            } else {
                return Response.status(Status.BAD_REQUEST).entity("Credentials \"" + credentialsID
                    + "\" are not set in their storage provider \"" + storageProviderID + "\".").build();
            }
