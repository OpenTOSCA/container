...
public class BoundsInterfaceOperationsResource {
	...
	private References getReferences() {
		...
		final List<String> ops = ToscaServiceHandler.getToscaEngineService().getToscaReferenceMapper().getBoundaryOperationsOfCSARInterface(this.csarID, this.serviceTemplateID, this.intName);

		for (final String op : ops) {
			refs.getReference().add(new Reference(Utilities.buildURI(this.uriInfo.getAbsolutePath().toString(), op), XLinkConstants.SIMPLE, op));
		}
