...
public class BoundsInterfacesResource {
    ...
    private References getReferences() {
        ...
        final List<String> interfaces = ToscaServiceHandler.getToscaEngineService().getToscaReferenceMapper()
                                                           .getBoundaryInterfacesOfCSAR(this.csarID);

        if (null != interfaces) {
            for (final String intf : interfaces) {
                refs.getReference()
                    .add(new Reference(Utilities.buildURI(this.uriInfo, intf), XLinkConstants.SIMPLE, intf));
            }
        }
