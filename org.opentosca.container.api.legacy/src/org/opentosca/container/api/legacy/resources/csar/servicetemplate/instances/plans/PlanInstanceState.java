...
public class PlanInstanceState {
	...
	public PlanInstanceState(final CSARID csarID, final QName serviceTemplateID, final int serviceTemplateInstanceId, final String correlationID) {
		this.csarID = csarID;
	...
	public Response getReferencesXML(@Context final UriInfo uriInfo) {
		...
		final PlanInvocationEvent event = CSARInstanceManagementHandler.csarInstanceManagement.getPlanForCorrelationId(this.correlationID);
		...
		builder.append("<PlanInstance planName=\"").append(event.getPlanName()).append("\" correlationID=\"").append(this.correlationID).append("\"><State>");
		if (finished) {
	...
	public Boolean hasFinished() {
		...
		// not finished
		else if ((null != CSARInstanceManagementHandler.csarInstanceManagement.getActiveCorrelations(this.csarID)) && CSARInstanceManagementHandler.csarInstanceManagement.getActiveCorrelations(this.csarID).contains(this.correlationID)) {
			return false;
		}
