...
/**
 * Main resource of the storage providers of the Core File Service. It shows only XLink references
 * to sub resources.<br />
 * <br />
...
public class StorageProvidersResource {
    ...
    /**
     * @param uriInfo
     * @return 200 (OK) with XLink references to available storage providers and default storage
     *         provider resources (and self reference).
     */
    @GET
    @Produces(ResourceConstants.LINKED_XML)
    public Response getReferences(@Context final UriInfo uriInfo) {

        final References refs = new References();

        refs.getReference().add(new Reference(Utilities.buildURI(uriInfo.getAbsolutePath().toString(), "Available"),
            XLinkConstants.SIMPLE, "Available"));
        refs.getReference().add(new Reference(Utilities.buildURI(uriInfo.getAbsolutePath().toString(), "Default"),
            XLinkConstants.SIMPLE, "Default"));
        refs.getReference()
