...
public class AvailableStorageProviderResource {
	...
	/**
	 * If {@code null} it's a not available storage provider and every method
	...
	/**
	 * Creates a {@link AvailableStorageProviderResource} of storage provider
	 * {@code storageProviderID}.
	 * 
	 * @param storageProviderID of storage provider.
	 */
	public AvailableStorageProviderResource(String storageProviderID) {
	 ...
	 * 
	 * @return 200 (OK) with entity {@link StorageProviderJaxb} containing the
	 *         storage provider that will be showed. If this storage provider is
	...
	/**
	 * Either sets this storage provider as the active one if {@code activate}
	 * is passed in {@code input} (body of a POST message) or deletes the
	 * credentials in this storage provider if {@code unset} is passed instead.
	 * 
	 * @param input
	 * 
	 * @return 200 (OK) - storage provider was set as the active one or
	 *         credentials were deleted in this storage provider.<br />
	 *         400 (bad request) - {@code activate} respectively {@code unset}
	 *         was not passed or storage provider has no credentials to delete.<br />
	 *         500 (internal server error) - setting this storage provider as
	 *         the active one or deleting credentials in this storage provider
	 *         failed.<br />
	...
	public Response setAsActiveOrUnsetCredentials(String input) throws UserException, SystemException {
		...
		if (input.equalsIgnoreCase("activate")) {
			...
			return Response.ok("Storage provider \"" + this.STORAGE_PROVIDER_ID + "\" was set as the active one.").build();
			
		} else if (input.equalsIgnoreCase("unset")) {
			
			boolean hasCredentials = this.CREDENTIALS_SERVICE.hasStorageProviderCredentials(this.STORAGE_PROVIDER_ID);
			
			if (hasCredentials) {
				this.CREDENTIALS_SERVICE.deleteCredentialsInStorageProvider(this.STORAGE_PROVIDER_ID);
				return Response.ok("Credentials were deleted in storage provider \"" + this.STORAGE_PROVIDER_ID + "\".").build();
			} else {
				return Response.status(Status.BAD_REQUEST).entity("Storage provider \"" + this.STORAGE_PROVIDER_ID + "\" has no credentials.").build();
			}
		...
		// AvailableStorageProviderResource.LOG.warn("A System Exception occured.",
		// exc);
		// } catch (UserException exc) {
		// AvailableStorageProviderResource.LOG.warn("An User Exception occured.",
		// exc);
		// }
