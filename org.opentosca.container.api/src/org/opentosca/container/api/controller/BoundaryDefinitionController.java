...
public class BoundaryDefinitionController {
	...
	public Response getInterface(@PathParam("name") final String name, @PathParam("csar") final String csar, @PathParam("servicetemplate") final String servicetemplate) {
		...
		final List<TExportedOperation> operations = this.getExportedOperations(csarContent.getCSARID(), QName.valueOf(servicetemplate), name);
		...
		dto.setOperations(operations.stream().map(o -> {
			...
			if (o.getPlan() != null) {
				...
				if (PlanTypes.BUILD.toString().equals(plan.getPlanType())) {
					...
					planUrl = this.uriInfo.getBaseUriBuilder().path("/csars/{csar}/servicetemplates/{servicetemplate}/buildplans/{buildplan}").build(csar, servicetemplate, plan.getId());
				} else {
					// ... else we assume it's a management plan
					planUrl = this.uriInfo.getBaseUriBuilder().path("/csars/{csar}/servicetemplates/{servicetemplate}/instances/:id/managementplans/{managementplan}").build(csar, servicetemplate, plan.getId());
				}
	...
	private List<TExportedOperation> getExportedOperations(final CSARID csarId, final QName serviceTemplate, final String interfaceName) {
		final Map<QName, List<TExportedInterface>> exportedInterfacesOfCsar = this.referenceMapper.getExportedInterfacesOfCSAR(csarId);
