	...
	private Response handleCsarUpload(final String filename, final InputStream is) {
		...
		try {
			...
			if (ModelUtils.hasOpenRequirements(csarId)) {
				...
				if (wc.isWineryRepositoryAvailable()) {
					...
					return Response.status(Response.Status.NOT_ACCEPTABLE).entity("{ \"Location\": \"" + wc.getServiceTemplateURI(serviceTemplate).toString() + "\" }").build();
				} else {
		...
		if (success) {
			...
			for (final QName serviceTemplate : serviceTemplates) {
				logger.info("Invoke IA deployment for service template \"{}\" of CSAR \"{}\"", serviceTemplate, csarId.getFileName());
				if (!this.controlService.invokeIADeployment(csarId, serviceTemplate)) {
					logger.error("Error deploying IA for service template \"{}\" of CSAR \"{}\"", serviceTemplate, csarId.getFileName());
					success = false;
				}
				logger.info("Invoke plan deployment for service template \"{}\" of CSAR \"{}\"", serviceTemplate, csarId.getFileName());
				if (!this.controlService.invokePlanDeployment(csarId, serviceTemplate)) {
					logger.error("Error deploying plan for service template \"{}\" of CSAR \"{}\"", serviceTemplate, csarId.getFileName());
					success = false;
				}
			}
		}
