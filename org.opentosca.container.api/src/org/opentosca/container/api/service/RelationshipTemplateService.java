...
/**
 * Provides data access functionality to retrieve relationship templates based on a
...
public class RelationshipTemplateService {
	 ...
	 * @return A collection of relationship templates stored within the given service
	 *         template.
	 */
	public List<RelationshipTemplateDTO> getRelationshipTemplatesOfServiceTemplate(String csarId, String serviceTemplateQName) {
		final CSARContent csarContent = this.csarService.findById(csarId);
		final List<String> relationshipTemplateIds = toscaEngineService
				.getRelationshipTemplatesOfServiceTemplate(csarContent.getCSARID(), QName.valueOf(serviceTemplateQName));
		...
		for (final String id : relationshipTemplateIds) {
			currentRelationshipTemplate = createRelationshipTemplate(csarContent.getCSARID(), QName.valueOf(serviceTemplateQName), id);
	 ...
	 *            The id of the relationship template we want to get and that belongs to the
	 *            specified service template
	 * @return The relationship template specified by the given id
	 * @throws NotFoundException
	 *             If the service template does not contain the specified relationship
	 *             template
	 */
	public RelationshipTemplateDTO getRelationshipTemplateById(String csarId, QName serviceTemplateQName, String relationshipTemplateId)
			throws NotFoundException {
		final CSARContent csarContent = this.csarService.findById(csarId);
	 ...
	 * @param relationshipTemplateId
	 *            the id of the relationship template to check for
	 * @return <code>true</code> when the CSAR contains the service template and the
	 *         service template contains the relationship template, otherwise
	 *         <code>false</code>
	 */
	public boolean hasRelationshipTemplate(String csarId, QName serviceTemplateQName, String relationshipTemplateId) {
		return this.getRelationshipTemplateIdsOfServiceTemplate(csarId, serviceTemplateQName.toString()).contains(relationshipTemplateId);
	}
	 ...
	 * @return
	 */
	public Document getPropertiesOfRelationshipTemplate(String csarId, QName serviceTemplateQName, String relationshipTemplateId) {
		final CSARContent csarContent = this.csarService.findById(csarId);
		final CSARID idOfCsar = csarContent.getCSARID();
		...
		final Document properties = this.toscaEngineService.getPropertiesOfRelationshipTemplate(idOfCsar, serviceTemplateQName, relationshipTemplateId);
	 ...
	 * @return
	 */
	private RelationshipTemplateDTO createRelationshipTemplate(CSARID csarId, QName serviceTemplateQName, String relationshipTemplateId) {
		...
		currentRelationshipTemplate.setRelationshipType(this.toscaEngineService
				.getRelationshipTypeOfRelationshipTemplate(csarId, serviceTemplateQName, relationshipTemplateId).toString());

		return currentRelationshipTemplate;
	}

	/**
	 * Gets a collection of relationship template ids associated to a given service
	 * template.
	 * 
	 * @param csarId
	 *            The id of the CSAR
	 * @param serviceTemplateQName
	 *            the QName of the service template within the given CSAR
	 * @return A collection of relationship template ids stored within the given service
	 *         template.
	 */
	private List<String> getRelationshipTemplateIdsOfServiceTemplate(String csarId, String serviceTemplateQName) {
		final CSARContent csarContent = this.csarService.findById(csarId);

		return toscaEngineService.getRelationshipTemplatesOfServiceTemplate(csarContent.getCSARID(),
