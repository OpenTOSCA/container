...
public class BpsConnector {
	 ...
	 *
	 * @param uri
	 *            the uri to the endpoint of WSO2 BPS
	 */
	 ...
	 *
	 * @param process
	 *            the process to deploy packaged for a WSO2 BPS
	 * @param uri
	 *            the URI of the WSO2 BPS
	 * @param user
	 *            the username for the WSO2 BPS
	 * @param pw
	 *            the password for the WSO2 BPS
	...
	public String deploy(final File process, final String uri, final String user, final String pw) throws Exception {
		...
		try {
			...
			BpsConnector.LOG
					.debug("Package name of PID is: " + info.getProcessInfo().getDeploymentInfo().getPackageName());
	 ...
	 *
	 * @param packageId
	 *            a String representing the packageId on a bps
	 * @param uri
	 *            the uri to the bps
	 * @param user
	 *            the user account
	 ...
	 *
	 * @param PID
	 *            The ProcessId (on a WSO2 BPS) of the WS-BPEL 2.0 process to
	 *            undeploy
	 * @param uri
	 *            the uri of the WSO2 BPS to undeploy from
	 * @param user
	 *            the username for the WSO2 BPS
	 * @param pw
	 *            the password for the WSO2 BPS
	 ...
	 * @param uri
	 *            the URI of the BPS the process has to be undeployed
	 * @param user
	 *            the userId to authenticate with the BPS
	 * @param pw
	 *            the password to for the given user account
	 ...
	 *             is thrown when connection failed
	 * @throws AuthenticationAdminAuthenticationExceptionException
	 *             is thrown when login was wrong
	 * @throws UnknownHostException
	 *             is thrown when no connection can be established
	 */
	private void login()
	 ...
	 * @param fileType
	 *            type of the process file
	 * @return Id Id of the uploaded package
	 * @throws RemoteException
	 *             is thrown when no connection is available
	 * @throws ProcessManagementException
	 *             is thrown when deployment had a failure
	 */
	...
	public List<String> getDeployedPackages(final String uri, final String user, final String pw) {
		...
		try {
			...
			while (count < pages) {
				...
				// make request
				final DeployedPackagesPaginated result = ((BPELPackageManagementServiceStub) this.setCookie(stub))
						.listDeployedPackagesPaginated(processReq);
	 ...
	 *
	 * @param stub
	 *            org.apache.axis2.client.Stub Object of the Axis2 library
	 ...
	 * @param uri
	 *            the URI to WSO2 BPS (2.1.2 - 3.0.0)
	 * @param user
	 *            the login username for the WSO2 BPS
	 * @param pw
	 *            the login password for the WSO2 BPS
	...
	public List<String> getAllPIDs(final String uri, final String user, final String pw) {
		...
		try {
			...
			// send request
			final ProcessIDList pidList = ((ProcessManagementServiceStub) this.setCookie(stub))
					.getAllProcesses(processReq);

			// check for case when there are no process deployed anymore
			if (pidList.getPid() == null) {
				BpsConnector.LOG
						.debug("Returned ProcessIDList from BPS is null, assuming no process is deployed on BPS");
	 ...
	 * @throws AxisFault
	 *             is thrown when initalizing the Axis2 stub fails
	 */
