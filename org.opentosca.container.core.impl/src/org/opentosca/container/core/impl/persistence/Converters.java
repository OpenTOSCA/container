...
public abstract class Converters {
	...
	public static ServiceInstance convert(final ServiceTemplateInstance object) {
		...
		ServiceTemplateInstanceProperty prop = object.getProperties().stream().filter(p -> p.getName().equalsIgnoreCase("xml")).collect(Collectors.reducing((a, b) -> null)).get();
		if (prop != null) {
	...
	public static NodeInstance convert(final NodeTemplateInstance object) {
		NodeInstance ni = new NodeInstance(object.getTemplateId(), "", object.getNodeType(), convert(object.getServiceTemplateInstance()));
		if (object.getId() != null) {
		...
		NodeTemplateInstanceProperty prop = object.getProperties().stream().filter(p -> p.getName().equalsIgnoreCase("xml")).collect(Collectors.reducing((a, b) -> null)).get();
		if (prop != null) {
		...
		try {
			...
			if (o.isPresent()) {
				...
				if (object.getProperties() != null) {
					...
					prop.setName("xml");
					prop.setType("xml");
					prop.setValue((String) converter.convertObjectValueToDataValue(object.getProperties(), null));
