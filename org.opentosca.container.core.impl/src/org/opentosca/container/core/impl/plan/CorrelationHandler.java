...
public class CorrelationHandler {
  ...
  public synchronized String getNewCorrelationID(final CSARID csarID, final QName serviceTemplateId,
    ...
    if (!this.mapCsarIdToServiceTemplateIdToSTInstanceIdToCorrelationToPublicPlan
        .containsKey(csarID)) {
    ...
    if (!this.mapCsarIdToServiceTemplateIdToSTInstanceIdToCorrelationToPublicPlan.get(csarID)
      ...
      this.mapCsarIdToServiceTemplateIdToSTInstanceIdToCorrelationToPublicPlan.get(csarID)
          .put(serviceTemplateId, new HashMap<Integer, Map<String, PlanInvocationEvent>>());
    }
    if (!this.mapCsarIdToServiceTemplateIdToSTInstanceIdToCorrelationToPublicPlan.get(csarID)
        .get(serviceTemplateId).containsKey(serviceTemplateInstanceId)) {
      this.mapCsarIdToServiceTemplateIdToSTInstanceIdToCorrelationToPublicPlan.get(csarID)
          .get(serviceTemplateId)
          .put(serviceTemplateInstanceId, new HashMap<String, PlanInvocationEvent>());
    }
    this.mapCsarIdToServiceTemplateIdToSTInstanceIdToCorrelationToPublicPlan.get(csarID)
        .get(serviceTemplateId).get(serviceTemplateInstanceId).put(corrID, event);
  ...
  public synchronized void correlateBuildPlanCorrToServiceTemplateInstanceId(final CSARID csarID,
    ...
    if (falseSTInstanceId != null) {
      final Map<String, PlanInvocationEvent> map =
          this.mapCsarIdToServiceTemplateIdToSTInstanceIdToCorrelationToPublicPlan.get(csarID)
              .get(serviceTemplateId).remove(falseSTInstanceId);
      this.mapCsarIdToServiceTemplateIdToSTInstanceIdToCorrelationToPublicPlan.get(csarID)
          .get(serviceTemplateId).put(correctSTInstanceId, map);
    }
  ...
  public TPlanDTO getPublicPlanForCorrelation(final String correlationID) {
    for (final CSARID csarID : this.mapCsarIdToServiceTemplateIdToSTInstanceIdToCorrelationToPublicPlan
        .keySet()) {
      for (final QName serviceTemplateId : this.mapCsarIdToServiceTemplateIdToSTInstanceIdToCorrelationToPublicPlan
          .get(csarID).keySet()) {
        for (final Integer serviceTemplateInstanceId : this.mapCsarIdToServiceTemplateIdToSTInstanceIdToCorrelationToPublicPlan
          ...
          if (this.mapCsarIdToServiceTemplateIdToSTInstanceIdToCorrelationToPublicPlan.get(csarID)
            ...
            final PlanInvocationEvent event =
                this.mapCsarIdToServiceTemplateIdToSTInstanceIdToCorrelationToPublicPlan.get(csarID)
                    .get(serviceTemplateId).get(serviceTemplateInstanceId).get(correlationID);
   ...
   * @return PublicPlan
   */
  public TPlanDTO getPlanDTOForCorrelation(final ServiceTemplateInstanceID instanceID,
      final String correlationID) {

    if (this.mapCsarIdToServiceTemplateIdToSTInstanceIdToCorrelationToPublicPlan
        .containsKey(instanceID.getCsarId())) {
      if (this.mapCsarIdToServiceTemplateIdToSTInstanceIdToCorrelationToPublicPlan
          .get(instanceID.getCsarId()).containsKey(instanceID.getServiceTemplateId())) {
        if (this.mapCsarIdToServiceTemplateIdToSTInstanceIdToCorrelationToPublicPlan
            .get(instanceID.getCsarId()).get(instanceID.getServiceTemplateId())
            .containsKey(instanceID.getInstanceID())) {
          if (this.mapCsarIdToServiceTemplateIdToSTInstanceIdToCorrelationToPublicPlan
              .get(instanceID.getCsarId()).get(instanceID.getServiceTemplateId())
              .get(instanceID.getInstanceID()).containsKey(correlationID)) {

            final PlanInvocationEvent event =
                this.mapCsarIdToServiceTemplateIdToSTInstanceIdToCorrelationToPublicPlan
                    .get(instanceID.getCsarId()).get(instanceID.getServiceTemplateId())
  ...
  public List<String> getActiveCorrelationsOfInstance(final ServiceTemplateInstanceID instanceID) {
    ...
    if (this.mapCsarIdToServiceTemplateIdToSTInstanceIdToCorrelationToPublicPlan
      ...
      if (this.mapCsarIdToServiceTemplateIdToSTInstanceIdToCorrelationToPublicPlan.get(csarID)
          .containsKey(stQName)) {
        if (this.mapCsarIdToServiceTemplateIdToSTInstanceIdToCorrelationToPublicPlan.get(csarID)
            .get(stQName).containsKey(stInstanceId)) {
          for (final String corr : this.mapCsarIdToServiceTemplateIdToSTInstanceIdToCorrelationToPublicPlan
              .get(csarID).get(stQName).get(stInstanceId).keySet()) {
            list.add(corr);
          }
        }
      }
