...
public class SOAPMessageGenerator implements EventHandler {
	 ...
	 * @return SOAPMessage
	 */
	public SOAPMessage createRequest(final CSARID csarID, final QName messageID, final List<TParameterDTO> params, final String correlationID) {
		...
		try {
			...
			for (final TParameterDTO para : params) {
				...
				if (para.getType().equalsIgnoreCase("correlation") || para.getName().equalsIgnoreCase("CorrelationID")) {
					...
					this.LOG.debug("Found csarEntrypoint Element! Put in instanceDataAPIUrl \"" + Settings.CONTAINER_API_LEGACY + "/" + csarID + "\".");
					final Name elementName = envelope.createName(para.getName(), "tosca", messageNS);
