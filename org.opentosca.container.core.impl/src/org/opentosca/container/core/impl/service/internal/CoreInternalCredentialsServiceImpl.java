...
import org.opentosca.container.core.impl.service.internal.credentials.CredentialsJPAStore;
import org.opentosca.container.core.model.credentials.Credentials;
import org.opentosca.container.core.service.internal.ICoreInternalCredentialsService;
import org.opentosca.container.core.service.internal.ICoreInternalFileStorageProviderService;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

/**
 * Provides a store and management functionalities for the credentials of the storage providers.
 *
 * Also has a connection to the storage providers: Credentials can be set / injected in a available
 * storage provider. If some conditions are fulfilled (see {@link #storeCredentials(Credentials)} a
 * stored credentials will be automatically set in the appropriate storage provider. Otherwise it
 * can be done manually by using {@link #setCredentialsInStorageProvider(String, String)}.
 * Credentials can be also stored if the appropriate storage provider is not available. Currently a
 * credentials consists of an identity and a key. You can get the name of the identity and key of a
 * certain storage provider by using {@link #getCredentialsIdentityName(String)} and
 * {@link #getCredentialsKeyName(String)}. For example, for AWS S3 the identity is the Access Key
 * and key is the Secret Access Key.
 *
 * Most of the methods of the credentials service are also accessible via OSGi console commands.
 *
 * @see ICoreInternalFileStorageProviderService
 *
 * @todo In future maybe other components needs also a store for credentials. Then we should extend
 *       the Credentials Service (and its model) to a central Credentials Service of OpenTOSCA.
 */
public class CoreInternalCredentialsServiceImpl implements ICoreInternalCredentialsService {
    ...
    public long storeCredentials(final Credentials credentials) throws UserException {
        ...
        if (!isValid) {
            throw new UserException(
                "Storing credentials not possible, because some required data are missing in credentials.");
        }
        ...
        if (storageProvider != null) {
            ...
            if (!needsCredentials) {
                throw new UserException("Credentials can't be stored, because available storage provider \""
                    + storageProviderID + "\" needs no credentials.");
            }
     ...
     * In a credentials the storage provider ID, the identity and the key must be set. A description is
     * optional.
     *
    ...
    private void setCredentialsInStorageProvider(final Credentials credentials) throws SystemException, UserException {
        ...
        if (storageProvider == null) {
            throw new SystemException(
                "Can set credentials in storage provider \"" + storageProviderID + "\", because it is not available.");
        }

        if (!storageProvider.needsCredentials()) {
            throw new UserException("Credentials were not set in storage provider \"" + storageProviderID
    ...
    public void deleteCredentialsInStorageProvider(final String storageProviderID) throws SystemException {
        ...
        if (storageProvider == null) {
            throw new SystemException("No credentials can be deleted, because storage provider \"" + storageProviderID
                + "\" is not available.");
        }
    ...
    public boolean hasStorageProviderCredentials(final String storageProviderID) throws SystemException {
        ...
        if (storageProvider == null) {
            throw new SystemException("Can't check if storage provider \"" + storageProviderID
                + "\" has credentials, because storage provider is not available.");
        }
    ...
    public boolean needsStorageProviderCredentials(final String storageProviderID) throws SystemException {
        ...
        if (storageProvider == null) {
            throw new SystemException("Can't check if storage provider \"" + storageProviderID
                + "\" needs credentials, because it is not available.");
        }
    ...
    public String getCredentialsIdentityName(final String storageProviderID) throws SystemException {
        ...
        if (storageProvider == null) {
            throw new SystemException("Can't get credentials identity name of storage provider \"" + storageProviderID
    ...
    public String getCredentialsKeyName(final String storageProviderID) throws SystemException {
        ...
        if (storageProvider == null) {
            throw new SystemException("Can't get credentials key name of storage provider \"" + storageProviderID
                + "\", because it is not available.");
        }
     ...
     * @return Storage provider {@code storageProviderID}. If it's not available {@code null}.
     */
     ...
     * If exactly one credentials for a storage provider is stored these credentials will be directly
     * set in the storage provider after binding.
     *
     ...
     * Deleting credentials in storage provider is not necessary on unbinding, because a unbound storage
     * provider has automatically lost is set / injected credentials.
     *
