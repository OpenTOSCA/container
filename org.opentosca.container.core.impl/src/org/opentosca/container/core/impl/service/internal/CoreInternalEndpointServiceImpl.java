 ...
 */
public class CoreInternalEndpointServiceImpl
    implements ICoreInternalEndpointService, CommandProvider {
  ...
  public List<WSDLEndpoint> getWSDLEndpoints(final QName portType, final CSARID csarId) {
     ...
     */
    final Query getWSDLEndpointsQuery = em.createNamedQuery(WSDLEndpoint.getWSDLEndpointByPortType);
  ...
  public WSDLEndpoint getWSDLEndpoint(final QName portType, final CSARID csarId) {
     ...
     */
    final Query getWSDLEndpointQuery = em.createNamedQuery(WSDLEndpoint.getWSDLEndpointByPortType);
  ...
  public void storeWSDLEndpoint(final WSDLEndpoint endpoint) {
    CoreInternalEndpointServiceImpl.LOG
        .debug("Storing WSDL Endpoint with CSARID: \"" + endpoint.getCSARId() + "\", portType: \""
            + endpoint.getPortType() + "\", IAName: \"" + endpoint.getIaName()
            + "\", NodeTypeImplementation: \"" + endpoint.getNodeTypeImplementation()
            + "\", URI: \"" + endpoint.getURI().toString() + "\"");
    ...
    if (!existsWSDLEndpoint(endpoint)) {
      ...
      CoreInternalEndpointServiceImpl.LOG
          .debug("The endpoint for \"{}\" is not stored. Thus store it.", endpoint.getPortType());
      em.persist(endpoint);
  ...
  public List<RESTEndpoint> getRestEndpoints(final URI anyURI, final CSARID csarId) {
     ...
     **/
    final Query getRestEndpointsQuery = em.createNamedQuery(RESTEndpoint.getEndpointForPath);
   ...
   * {@Inheritdoc}
   */
  public RESTEndpoint getRestEndpoint(final URI anyURI, final restMethod method,
      final CSARID csarId) {
    /**
     ...
     */
    final Query getRestEndpointQuery =
        em.createNamedQuery(RESTEndpoint.getEndpointForPathAndMethod);
  ...
  public void storeRESTEndpoint(final RESTEndpoint endpoint) {
    CoreInternalEndpointServiceImpl.LOG.debug(
        "Storing REST Endpoint with Path : \"{}\", STID: \"{}\"", endpoint.getPath(),
  ...
  public boolean endpointExists(final URI uri, final CSARID csarId) {
    final Query existsRestEndpointQuery = em.createNamedQuery(RESTEndpoint.getEndpointForUri);
    existsRestEndpointQuery.setParameter("uri", uri);
    existsRestEndpointQuery.setParameter("csarId", csarId);

    final Query existsWsdlEndpointQuery = em.createNamedQuery(WSDLEndpoint.getWSDLEndpointByUri);
  ...
  public void _endpoint_clear_wsdl(final CommandInterpreter commandInterpreter) {
    if (!em.getTransaction().isActive())
      em.getTransaction().begin();
    final Query query = em.createQuery("DELETE FROM WSDLEndpoint");
  ...
  public void _endpoint_clear_rest(final CommandInterpreter commandInterpreter) {
    if (!em.getTransaction().isActive())
      em.getTransaction().begin();
    final Query query = em.createQuery("DELETE FROM RESTEndpoint");
  ...
  public void _endpoint_add_dummy_rest(final CommandInterpreter commandInterpreter) {
    try {
      final RESTEndpoint endpoint = new RESTEndpoint(new URI("http://www.balbla.com/xyz"),
          restMethod.GET, new CSARID("mockup.example.test"));
  ...
  public void _endpoint_add_dummy_wsdl(final CommandInterpreter commandInterpreter) {
    try {
      ...
      final WSDLEndpoint endpoint =
          new WSDLEndpoint(new URI("http://blabla/"), new QName("somePort"),
  ...
  public void _endpoint_show_rest(final CommandInterpreter commandInterpreter) {
    ...
    final Query query = em.createQuery("SELECT e FROM RESTEndpoint e");
    @SuppressWarnings("unchecked")
    final List<RESTEndpoint> queryResults = query.getResultList();
    for (final RESTEndpoint e : queryResults)
      commandInterpreter
          .println("SeriviceTemplateID: " + e.getCSARId().getFileName() + " URI: " + e.getURI());

  }

  public void _endpoint_show_wsdl(final CommandInterpreter commandInterpreter) {
    final Query query = em.createQuery("SELECT e FROM WSDLEndpoint e");
    @SuppressWarnings("unchecked")
    ...
    for (final WSDLEndpoint e : queryResults) {
      ...
      commandInterpreter
          .println("PlanId: " + (e.getPlanId() == null ? "" : e.getPlanId().toString()));
      commandInterpreter.println("NodeTypeImpl: " + (e.getNodeTypeImplementation() == null ? ""
  ...
  public void removeEndpoints(final CSARID csarId) {
    ...
    final Query queryRestEndpoints =
        em.createQuery("SELECT e FROM RESTEndpoint e where e.csarId = :csarId");
    ...
    final Query queryWsdlEndpoints =
        em.createQuery("SELECT e FROM WSDLEndpoint e where e.csarId = :csarId");
  ...
  public WSDLEndpoint getWSDLEndpointForPlanId(final CSARID csarId, final QName planId) {
    ...
    final Query queryWSDLEndpoint = em
        .createQuery("SELECT e FROM WSDLEndpoint e where e.csarId= :csarId and e.PlanId = :planId");
  ...
  @Override
  public WSDLEndpoint getWSDLEndpointForIa(final CSARID csarId, final QName nodeTypeImpl,
      final String iaName) {
    WSDLEndpoint endpoint = null;
    final Query queryWSDLEndpoint = em.createQuery(
  ...
  @Override
  public List<WSDLEndpoint> getWSDLEndpointsForNTImplAndIAName(final QName nodeTypeImpl,
      final String iaName) {
    final ArrayList<WSDLEndpoint> endpoints = new ArrayList<>();
    final Query queryWSDLEndpoint = em.createQuery(
        "SELECT e FROM WSDLEndpoint e where e.IaName = :IaName and e.NodeTypeImplementation = :nodeTypeImpl");
    queryWSDLEndpoint.setParameter("IaName", iaName);
    queryWSDLEndpoint.setParameter("nodeTypeImpl", nodeTypeImpl);
  ...
  public List<WSDLEndpoint> getWSDLEndpoints() {
    final ArrayList<WSDLEndpoint> endpoints = new ArrayList<>();
    final Query queryWSDLEndpoint = em.createQuery("SELECT e FROM WSDLEndpoint e");
