 ...
 * {@link AbstractJCloudsFileStorageProvider}, so it uses jclouds to access the file system. This
 * class primarily contains meta data of the storage provider and some necessary jclouds
 * configuration like the location of the file system-based blobstore.
 *
 * A CSAR file will be stored at the following path:
 * {@code <BlobStorePath>/<containerName>/<csarID>/<relPathOfFileToCSARRoot>}
 *
 * The blobstore path is defined in {@link #CSAR_STORE_PATH}. By default, the container name is
 * {@code org.opentosca.csars}. Setting a new container name or getting the current one is possible
 * via OSGi console commands.
 *
 * Note: If you have stored files on this storage provider and changes the container name (creates
 * new container) these files can't be found anymore, because the storage provider searches for the
 * files in the new container. Thus, for retrieving the files again you must switch back to the
 * previous container.
 */
public class CoreInternalFileStorageProviderFileSystemServiceImpl extends AbstractJCloudsFileStorageProvider
                                                                  implements ICoreInternalFileStorageProviderService,
                                                                  CommandProvider {

    /**
     * Storage provider ID.<br />
     * It's a storage provider based on jclouds, so the ID must be equal to the jclouds file system API
     * ID.
     */
    ...
    public void _setFilesystemContainerName(final CommandInterpreter ci) {
        ...
        if (!inputInvalid) {
            ...
            ci.println("Container name \"" + containerName + "\" was set in storage provider \""
