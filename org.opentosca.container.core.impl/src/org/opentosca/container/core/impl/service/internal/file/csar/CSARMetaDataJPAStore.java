...
public class CSARMetaDataJPAStore {
   ...
   * @param fileToStorageProviderIDMap - file to storage provider ID mapping of all files of the
   *        CSAR. Each file path must be given relative to the CSAR root.
  ...
  public void storeCSARMetaData(final CSARID csarID, final Set<Path> directories,
    ...
    final CSARContent csar =
        new CSARContent(csarID, directories, fileToStorageProviderIDMap, toscaMetaFile);
  ...
  public boolean isCSARMetaDataStored(final CSARID csarID) {
    ...
    final CSARContent csar = em.find(CSARContent.class, csarID);
  ...
  public Set<CSARID> getCSARIDsMetaData() {
    ...
    initJPA();
    final Query getCSARIDsQuery = em.createNamedQuery(CSARContent.getCSARIDs);
   ...
   * @throws UserException if meta data of file {@code fileRelToCSARRoot} in CSAR {@code CSARID}
   *         were not found.
   */
  public void storeFileStorageProviderIDOfCSAR(final CSARID csarID, final Path fileRelToCSARRoot,
    ...
    CSARMetaDataJPAStore.LOG.debug(
        "Setting storage provider \"{}\" in meta data of file \"{}\" in CSAR \"{}\"...",
        storageProviderID, fileRelToCSARRoot, csarID);

    initJPA();

    final Query storeStorageProviderIDByFileAndCSARIDQuery =
        em.createNamedQuery(CSARContent.storeStorageProviderIDByFileAndCSARID);
    ...
    if (updatedFiles > 0) {
      ...
      throw new UserException("Meta data of file \"" + fileRelToCSARRoot + "\" of CSAR \"" + csarID
          + "\" were not found.");
  ...
  public Map<Path, String> getFileToStorageProviderIDMap(final CSARID csarID) throws UserException {
    CSARMetaDataJPAStore.LOG
        .debug("Retrieving file to storage provider mapping meta data of CSAR \"{}\"...", csarID);
    initJPA();
    final Query getFileToStorageProviderIDMapQuery =
        em.createNamedQuery(CSARContent.getFileToStorageProviderIDMapByCSARID);
    getFileToStorageProviderIDMapQuery.setParameter("csarID", csarID);

    @SuppressWarnings("unchecked")
    final List<Object[]> fileToStorageProviderIDEntries =
        getFileToStorageProviderIDMapQuery.getResultList();
