...
public class ResourceAccess {
  ...
  /**
   * Allows accessing resources retrieved from the classpath through a classloader as {@link Path Paths} with all the benefits that entails.
   * Note that {@link Path} instances returned from {@link #resolvedPath()} are not necessarily compatible to the old {@link java.io.File} API.
   *
   * @param resource The URL of a resource returned by the classloader.
   * @throws IOException If a filesystem needed to be constructed for proper access to the resource and construction or retrieval of a cached instance failed.
