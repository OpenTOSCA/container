...
public interface CsarStorageService {
     ...
     * @throws SystemException if unpacking the CSAR failed, access to a directory was denied while getting files and
     *                         directories in unpack directory or if an error occurred during storing to disk.
    ...
    /**
     * <p>
     * Deletes the CSAR belonging to the given {@link CsarId} from storage. That includes deleting it's metadata from
     * the database.
     * <p>
     * If an error occurs during the deletion of files, the metadata stays in the database. Warning: this is a process
     * that can not be compensated. As such errors can leave the CSAR in an inconsistent state.
     *
    ...
    /**
     * Exports a stored CSAR {@code csarID}.<br /> First, it creates a directory structure and copies all files of the
     * CSAR to a sub directory of a Temporary directory. Finally the content of this directory will be compressed as a
     * CSAR file. The created CSAR file is located directly in that temporary directory.<br /> <br/>
     * <p>
     * Note: At the end or if an error occurred the directory that contains the content of the CSAR file will be deleted
     * (if necessary).
     *
