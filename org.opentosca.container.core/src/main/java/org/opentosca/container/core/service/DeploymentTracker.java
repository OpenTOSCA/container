...
public interface DeploymentTracker {
   ...
   * @return The deployment state of the given Csar, <tt>null</tt> if no state is available for the
   * given Csar
   */
  public DeploymentProcessState getDeploymentState(CsarId csar);

  /**
   * Stores the given information about the encapsulated implementation artifact into the database.
   * Previous deploymentInfo will be overwritten!
   *
  ...
  /**
   * Retrieve the stored deployment information for a specific implementation artifact identified by
   * it's relative path within the Csar and it's parent Csar.
   *
  ...
  /**
   * Store the given Deployment Information for a plan, overwriting any already stored information for
   * the Plan encapsulated in the Deployment Information object.
   *
   ...
   * @return The plan deployment information associated with the given Csar and plan, <tt>null</tt> if no such plan is known.
   */
