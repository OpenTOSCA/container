...
/**
 * This class stores the mapping for Consolidated Policies. A CSARID is mapped to the amount of
 * Templates (ServiceTemplate or NodeTemplate) which can contain Policies and the actual
...
public class CsarIDToPolicies implements Map<CSARID, Map<QName, Policies>> {
    ...
    public void put(final CSARID csarID, final QName templateID, final Policies policies) {
        ...
        if (null != this.csarIDToTemplateToPolicies.get(csarID).get(templateID)) {
            this.LOG.warn("There are Consolidated Policies stored already for the CSARID \"" + csarID
                + " and TemplateID \"" + templateID + "\". Thus do overwrite the Consolidated Policies.");
        }
    ...
    public Policies get(final CSARID csarID, final QName templateID) {
        ...
        if (!this.csarIDToTemplateToPolicies.containsKey(csarID)) {
            ...
            this.LOG.error("There are no informations stored for the CSARID \"" + csarID + "\" and the TemplateID \""
                + templateID + "\".");
        } else {
