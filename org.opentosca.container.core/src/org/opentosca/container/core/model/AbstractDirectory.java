...
/**
 * Abstract class of a directory. Provides methods for browsing the directory
 * and getting their meta data.
 */
public abstract class AbstractDirectory implements IBrowseable {
	 ...
	 */
	public AbstractDirectory(final String directoryReference, final Set<String> includePatterns, final Set<String> excludePatterns, final boolean fileArtifact) {
	...
	public AbstractFile getFile(final String relPathOfFile) {
		...
		if (file != null) {
			...
			if (this.getIncludePatterns().isEmpty() && this.getExcludePatterns().isEmpty()) {
				...
				final boolean matchesPatterns = PatternMatcher.isFileMatchesPatterns(file, this.getIncludePatterns(), this.getExcludePatterns());
