...
/**
 * Matches files at an artifact reference against include and exclude patterns.
 * An include pattern includes certain files at a reference in the artifact. By
 * analogy, an exclude pattern excludes files from the artifact. A pattern must
...
public class PatternMatcher {
	 ...
	 */
	public static boolean isFileMatchesPatterns(final AbstractFile file, final Set<String> includePatterns, final Set<String> excludePatterns) {

		final Set<AbstractFile> files = new HashSet<>();
		files.add(file);

		final Set<AbstractFile> matches = PatternMatcher.findFilesMatchesPatterns(files, includePatterns, excludePatterns);
	 ...
	 */
	public static Set<AbstractFile> findFilesMatchesPatterns(final Set<AbstractFile> files, final Set<String> includePatterns, final Set<String> excludePatterns) {
		...
		if ((includePatterns != null) && !includePatterns.isEmpty()) {
			for (final String includePattern : includePatterns) {
				...
				final Set<AbstractFile> filesMatchingPattern = PatternMatcher.getSubsetMatchesPattern(filesToMatch, includePattern);
