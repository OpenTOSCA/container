...
@NamedQueries({@NamedQuery(name = CSARContent.getCSARIDs, query = CSARContent.getCSARIDsQuery),
    ...
    @NamedQuery(name = CSARContent.getDirectoriesByCSARID,
        query = CSARContent.getDirectoriesByCSARIDQuery)})
...
public class CSARContent implements IBrowseable {
  ...
  protected static final String getCSARIDsQuery =
      "SELECT t.csarID FROM " + CSARContent.CSAR_TABLE_NAME + " t";
  ...
  protected static final String getFileToStorageProviderIDMapByCSARIDQuery =
      "SELECT KEY(u), VALUE(u) FROM " + CSARContent.CSAR_TABLE_NAME
          + " t JOIN t.fileToStorageProviderIDMap u WHERE t.csarID = :csarID";

  /**
   * For storing / updating the storage provider ID of a file in CSAR we must use a native SQL
   * query, because JPQL update queries doesn't work on Maps.
   */
  ...
  protected static final String storeStorageProviderIDByFileAndCSARIDQuery =
      "UPDATE " + CSARContent.CSAR_FILES_TABLE_NAME
          + " SET storageProviderID = ? WHERE file = ? AND csarID = ?";

  public static final String getDirectoriesByCSARID =
      CSARContent.CSAR_TABLE_NAME + ".getDirectoriesByCSARID";
   ...
   */
  @Transient
  private final String CSAR_DEFINITIONS_DIR_REL_PATH =
      Settings.getSetting("csarDefinitionsRelPath");
   ...
   */
  @ElementCollection
  @CollectionTable(name = CSARContent.CSAR_FILES_TABLE_NAME,
      joinColumns = @JoinColumn(name = "csarID"))
   ...
   */
  @ElementCollection
  @CollectionTable(name = CSARContent.CSAR_DIRECTORIES_TABLE_NAME,
      joinColumns = @JoinColumn(name = "csarID"))
  ...
  /**
   * @param fileExtension
   * @return All files with extension {@code fileExtension} in directory "IMPORTS" of this CSAR as
   *         Set of {@code AbstractFile}.
   */
  private Set<AbstractFile> getImportFiles(final String fileExtension) {

    final Set<AbstractFile> importFiles = new HashSet<>();

    CSARContent.LOG.debug("Retrieving import file(s) with extension \"{}\" in CSAR \"{}\"...",
        fileExtension, this.csarID);

    final AbstractDirectory importsDirectory = this.getDirectory(this.IMPORTS_DIR_REL_PATH);
    ...
    CSARContent.LOG.debug("{} import file(s) with extension \"{}\" were found in CSAR \"{}\".",
        importFiles.size(), fileExtension, this.csarID);
   ...
   *         supported.
   */
  public AbstractArtifact resolveArtifactReference(final String artifactReference)
      throws UserException, SystemException {

    // no patterns, so we pass empty sets (immutable to avoid unnecessary
    // object creations)
    return this.resolveArtifactReference(artifactReference, Collections.<String>emptySet(),
   ...
   *        relative URI points to a file / directory in this CSAR. If you pass an empty string it's
   *        means the CSAR root, so you get an artifact that represents the complete CSAR.
   * @param includePatterns to include only certain files in the artifact. No include patterns must
   *        be passed by an empty set.
  ...
  public AbstractArtifact resolveArtifactReference(final String artifactReference,
    ...
    try {
      ...
      CSARContent.LOG.debug("Artifact reference \"{}\" is a valid URI.",
          artifactReferenceTrimed.toString());

      AbstractArtifact artifact = null;

      if (CSARArtifact.fitsArtifactReference(artifactReferenceTrimed)) {
        artifact = new CSARArtifact(artifactReferenceTrimed, includePatterns, excludePatterns,
            this.csarID, this.directories, this.fileToStorageProviderIDMap);
        // if further AbstractArtifact implementations exists, we
        // can check here if they fits
      } else {
        throw new SystemException(
            "Artifact reference \"" + artifactReferenceTrimed + "\" is not supported.");
      }

      CSARContent.LOG.debug("Resolving artifact reference \"{}\" completed.",
          artifactReferenceTrimed);

      return artifact;

    } catch (final URISyntaxException exc) {
      throw new UserException(
          "Artifact reference \"" + artifactReference + "\" is not a valid URI.", exc);
    }
  ...
  public Set<AbstractFile> getTOSCAsInDefinitionsDir() {
    ...
    CSARContent.LOG.debug("Retrieving TOSCA files in directory \"{}\" of CSAR \"{}\"...",
        this.CSAR_DEFINITIONS_DIR_REL_PATH, this.csarID);

    Set<AbstractFile> toscasInDefinitionsDir = new HashSet<>();
    ...
    if (definitionsDir != null) {
      ...
      CSARContent.LOG.warn("Directory \"{}\" was not found in CSAR \"{}\".",
          this.CSAR_DEFINITIONS_DIR_REL_PATH, this.csarID);
    }
   ...
   *         If no root TOSCA path is specified in the TOSCA meta file (attribute
   *         "Entry-Definitions") or path points to a non-existent file {@code null}.
   */
  public AbstractFile getRootTOSCA() {
    ...
    if (relPathOfRootTOSCA != null) {
      ...
      if (rootTOSCA != null) {
        CSARContent.LOG.debug("Root TOSCA exists at \"{}\" in CSAR \"{}\".", rootTOSCA.getPath(),
            this.csarID);
      } else {
        CSARContent.LOG.warn(
            "Root TOSCA path \"{}\" specified in TOSCA meta file of CSAR \"{}\" points to a non-existing file.",
            relPathOfRootTOSCA, this.csarID);
      }

    } else {
      CSARContent.LOG.warn("Root TOSCA path is not specified in TOSCA meta file of CSAR \"{}\".",
          this.csarID);
    }
  ...
  /**
   * @return Description of this CSAR. If no description is specified in TOSCA meta file
   *         {@code null}.
   */
  ...
  /**
   * @return Picture that visualizes the topology of this CSAR as {@code AbstractFile}. If no
   *         topology picture path is specified in TOSCA meta file (attribute "Topology") or path
   *         points to a non-existent file {@code null}.
   */
  public AbstractFile getTopologyPicture() {
    ...
    if (topologyPictureRelPath != null) {
      ...
      if (topologyPicture != null) {
        CSARContent.LOG.debug("Topology picture exists at \"{}\" in CSAR \"{}\".",
            topologyPicture.getPath(), this.csarID);
      } else {
        CSARContent.LOG.warn(
            "Topology picture path specified in TOSCA meta file of CSAR \"{}\" points to a non-existing file.",
            this.csarID);
      }
    } else {
      CSARContent.LOG.warn(
          "Topology picture path is not specified in TOSCA meta file of CSAR \"{}\".", this.csarID);
    }
