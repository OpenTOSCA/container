...
public class IdConverter {
  ...
  private static Integer nodeInstanceUriToID(String nodeInstanceIDPath) {
    ...
    if (paths[paths.length - 2].equals("Instances")
        & paths[paths.length - 4].equals("NodeTemplates")) {
      try {
  ...
  private static Integer relationInstanceUriToID(String relationInstanceIDPath) {
    ...
    // if the paths are at the end are correct we assume a good URI
    if (paths[paths.length - 2].equals("Instances")
        & paths[paths.length - 4].equals("RelationshipTemplates")) {
      try {
        return Integer.parseInt(paths[paths.length - 1]);
      } catch (NumberFormatException e) {
        return null;
  ...
  /**
   * Checks if <code>uri</code> is a valid serviceInstanceID-URI in the context of the
   * InstanceDataAPI this means that it has one of the following formats f.ex. with ID=12345
   *
   * <pre>
