...
public class NodeInstance {
	...
	protected final static String getNodeInstancesQuery = "select n from NodeInstance n where" + " n.id = COALESCE(:internalID, n.id) AND" + " n.nodeTemplateName = COALESCE(:nodeTemplateName, n.nodeTemplateName) AND" + " n.serviceInstance.id = COALESCE(:internalServiceInstanceID, n.serviceInstance.id) AND" + " n.nodeTemplateID = COALESCE(:nodeTemplateID, n.nodeTemplateID)";
	 ...
	 * set automatically.
	 *
	 * @param nodeTemplateID - the nodeTemplateID specified by the Namespace and
	 *            the ID value of the NodeTemplate
	 * @param nodeTemplateName - the name of the nodeTemplate
	 */
	public NodeInstance(final QName nodeTemplateID, final String nodeTemplateName, final QName nodeTypeOfNodeTemplate, final ServiceInstance serviceInstance) {
		super();
	...
	private void setNodeInstanceID() {
		try {
			this.nodeInstanceID = new URI(Settings.CONTAINER_API + "/CSARs/" + this.serviceInstance.getCSAR_ID() + "/ServiceTemplates/" + URLEncoder.encode(URLEncoder.encode(this.serviceInstance.getServiceTemplateID().toString(), "UTF-8"), "UTF-8") + "/Instances/" + this.serviceInstance.getDBId() + "/NodeTemplates/" + this.nodeTemplateName + "/Instances/" + this.id);			
	...
	public String toString() {
		return "NodeInstance [id=" + this.id + ", nodeInstanceID=" + this.nodeInstanceID + ", nodeTemplateID=" + this.nodeTemplateID + ", nodeTemplateName=" + this.nodeTemplateName + ", created=" + this.created + ", serviceInstance=" + this.serviceInstance + ", properties=" + this.properties + "]";
	}
