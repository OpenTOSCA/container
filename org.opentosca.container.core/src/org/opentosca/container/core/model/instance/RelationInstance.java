...
@NamedQueries({@NamedQuery(name = RelationInstance.getRelationInstances, query = RelationInstance.getRelationInstancesQuery)})
public class RelationInstance {
	...
	protected final static String getRelationInstancesQuery = "select n from RelationInstance n where" + " n.id = COALESCE(:internalID, n.id) AND" + " n.relationshipTemplateName = COALESCE(:relationshipTemplateName, n.relationshipTemplateName) AND" + " n.serviceInstance.id = COALESCE(:internalServiceInstanceID, n.serviceInstance.id) AND" + " n.relationshipTemplateID = COALESCE(:relationshipTemplateID, n.relationshipTemplateID)";
	 ...
	 * set automatically.
	 *
	 * @param relationshipTemplateID - the relationshipTemplateID specified by
	 *            the Namespace and the ID value of the RelationshipTemplate
	 * @param relationshipTemplateName - the name of the nodeTemplate
	 */
	public RelationInstance(final QName relationshipTemplateID, final String relationshipTemplateName, final QName relationshipTypeOfRelationshipTemplate, final ServiceInstance serviceInstance, final NodeInstance sourceInstanceID, final NodeInstance targetInstanceID) {
		super();
	...
	public void setState(final QName state) {
		this.state = state;
	}
	...
	private void setRelationInstanceID() {
		try {
			this.relationInstanceID = new URI(Settings.CONTAINER_API + "/CSARs/" + this.serviceInstance.getCSAR_ID() + "/ServiceTemplates/" + URLEncoder.encode(URLEncoder.encode(this.serviceInstance.getServiceTemplateID().toString(), "UTF-8"), "UTF-8") + "/Instances/" + this.serviceInstance.getDBId() + "/RelationshipTemplates/" + this.relationshipTemplateName + "/Instances/" + this.id);
	...
	public String toString() {
		return "RelationInstance [id=" + this.id + ", relationInstanceID=" + this.relationInstanceID + ", relationshipTemplateID=" + this.relationshipTemplateID + ", relationshipTemplateName=" + this.relationshipTemplateName + ", created=" + this.created + ", serviceInstance=" + this.serviceInstance + ", properties=" + this.properties + "]";
	}
