...
/**
 * This interface of the Core Internal File Service defines methods for storing, getting, moving and
 * deleting CSAR files, moving files or directories contained in CSAR files, getting available and
 * ready storage providers, getting and setting the active storage provider, getting the default
 * storage provider.
 *
 * @see CSARContent
 */
public interface ICoreInternalFileService {
    ...
    /**
     *
     * @return IDs of available storage providers which have no unsatisfied requirements (ready) and
     *         therefore can be used.<br />
     *
     ...
     * @return {@code true} if storage provider {@code storageProviderID} is available and has no
     *         unsatisfied requirements, so it can be used. Otherwise {@code false} will be
     *         returned.<br />
     *         Usually a requirement of a storage provider are the credentials (exception: file system
     *         storage provider) which must be set in the storage provider.
     */
    ...
    /**
     * @return ID of active storage provider that will used for storing a CSAR and as target for moving
     *         a CSAR or a file / directory contained in a CSAR.
     */
    public String getActiveStorageProvider();

    /**
     * @return Friendly name of the storage provider {@code storageProviderID}. If
     *         {@code storageProviderID} is not available {@code null}.
     */
    public String getStorageProviderName(String storageProviderID);

    /**
     * Sets storage provider {@code storageProviderID} as the active storage provider that will used for
     * storing a CSAR and as target for moving a CSAR or file / directory contained in a CSAR.<br />
     * <br />
     * Notes:<br />
     * 1) After the start of the Container no active storage provider is set.<br />
     * 2) If the active storage provider goes unavailable, this setting will be cleared (no active
     * storage provider is set).<br />
     * 3) By passing {@code null} you can manually set no active storage provider.
     *
     *
     * @param storageProviderID of the storage provider to set as the active one.
     * @throws UserException if storage provider {@code storageProviderID} is not available.
     */
    public void setActiveStorageProvider(String storageProviderID) throws UserException;

    /**
     * @return ID of the default storage provider. It will be used for storing a CSAR and as the target
     *         for moving a CSAR or a file / directory of a CSAR if no active storage storage provider
     *         is set or it's not ready. If the default storage provider is also not available and ready
     *         the operation fails.<br />
     ...
     * Note 3: If an error occurred during storing the unpack directory will be also deleted (if CSAR
     * was already unpacked).
     *
     ...
     *
     * @throws UserException if {@code csarFile} does not exist, is not a valid ZIP file, is already
     *         stored or it's content is invalid, e.g. contains no TOSCA file in {@code Definitions}
     *         directory or has an invalid TOSCA meta file.
     *
    ...
    /**
     * Moves a CSAR {@code csarID} from it's storage provider(s) to the active storage provider (target
     * storage provider).<br />
     * <br />
     * First, the files meta data of CSAR {@code csarID} will be retrieved to check if the CSAR is
     * stored and to get the storage provider(s) of the files of the CSAR. Then all files will be
     * determined that must be moved to the target storage provider. If no files were found (completely
     * stored on target storage provider), the move process is completed. Otherwise the files will be
     * moved successively to the target storage provider.<br />
     * If an error occurred while moving files, the process will be canceled respectively further files
     * will not be moved anymore.<br />
     * <br />
     * Note: If no active storage provider is set (e.g. directly after the start of the container) or
     * it's not ready, the default storage provider will be used instead, if it's available and ready.
     *
     * @param csarID of CSAR.
    ...
    /**
     * Moves the file / directory {@code fileOrDirRelToCSARRoot} of CSAR {@code csarID} from it's
     * storage provider(s) to the active storage provider.<br />
     * <br />
     * First, the files and directories meta data of CSAR {@code csarID} will be retrieved to check if
     * the CSAR is stored, given {@code fileOrDirRelToCSARRoot} is a existent file or directory and to
     * get the storage provider of the file respectively the storage providers of all files in the
     * directory to move. If file / directory exists, all files will be determined that must be moved to
     * the target storage provider. If no files were found (completely stored on target storage
     * provider), the move process is completed. Otherwise the files will be moved successively to the
     * target storage provider.<br />
     * If an error occurred while moving files, the process will be canceled respectively further files
     * will not be moved anymore.<br />
     * <br />
     * Note 1: If no active storage provider is set (e.g. directly after the start of the container) or
     * it's not ready, the default storage provider will be used instead, if it's available and
     * ready.<br />
     * Note 2: Moving a directory means that all files in the directory will be moved. Directories are
     * only stored as meta data.
     *
     * @param csarID of CSAR
     * @param fileOrDirRelToCSARRoot - path relative to CSAR root of a file or directory in CSAR
     *        {@code csarID}.
     *
     * @see CoreInternalFileServiceImpl#findFilesToMove
     ...
     * @see CoreInternalFileServiceImpl#moveFilesToStorageProvider
     *
     * @throws UserException if CSAR {@code csarID} was not found or {@code fileOrDirRelToCSARRoot} does
     *         not exist in CSAR {@code csarID}.
     * @throws SystemException if a required storage provider is not available and ready or an error
     *         occurred during moving files.
     *
     */
     ...
     * <br />
     * The CSAR IDs of all stored CSAR will be retrieved and then each CSAR deleted by using
     * {@link ICoreInternalFileService#deleteCSAR(CSARID)}.<br />
