...
/**
 * This interface of a storage provider provides methods for getting the ID and name of the storage
 * provider, checking if the storage provider is ready, storing, getting and deleting files on the
 * storage provider, getting the size of files on the storage provider, managing credentials of the
 * storage provider and getting the storage provider-specific identity and key names of credentials.
 */
public interface ICoreInternalFileStorageProviderService {
    ...
    /**
     *
     * @return {@code true} if all requirements of this storage provider are satisfied so it can be
     *         used, otherwise {@code false}.
     */
    public boolean isStorageProviderReady();

    /**
     * Stores the file {@code absFilePath} as {@code relFilePathOnProvider} on this storage
     * provider.<br />
     * If the file already exists, it will be overwritten.
     *
     * @param absFilePath - absolute path of file.
     * @param relFilePathOnProvider - relative path where the file should be stored on storage provider.
     * @throws SystemException if {@code absFilePath} doesn't points to an existing file or an error
     *         occurred during storing.
     */
     ...
     * @param relFilePathOnProvider - relative path where the file should be stored on the storage
     *        provider.
     ...
     * {@code targetAbsFilePath}.<br />
     * If file {@code targetAbsFilePath} not exists, it will be created. Otherwise it will be
     * overwritten.
     *
     * @param relFilePathOnProvider - relative path of file on storage provider
     * @param targetAbsFilePath - local absolute file path.
     * @throws SystemException if {@code relFilePathOnProvider} was not found on storage provider,
     *         creating or writing to file {@code targetAbsFilePath} failed or an error occurred during
     *         retrieving.
     */
    ...
    /**
     *
     * Gets the input stream of file {@code relFilePathOnProvider} from this storage provider.
     *
     * @param relFilePathOnProvider - relative path of file on storage provider.
     * @return {@link InputStream} of file {@code relFilePathOnProvider}.
     * @throws SystemException if {@code relFilePathOnProvider} was not found on storage provider or an
    ...
    /**
     * Deletes the file {@code relFilePathOnProvider} on this storage provider, if it exists (deleting a
     * non-existent file is also a successful execution).
     *
     * @param relFilePathOnProvider - relative path of file on storage provider.
    ...
    /**
     * @return {@code true} if storage provider needs credentials, otherwise {@code false}.
     */
    ...
    /**
     *
     * @return ID of set credentials. If storage provider has currently no credentials {@code null}.
     */
