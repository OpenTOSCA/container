...
public class IAEnginePluginDockerComposeServiceImpl implements IIAEnginePluginService {
	...
	public URI deployImplementationArtifact(final CSARID csarId, final QName nodeTypeImplementationID, final QName artifactTypeQName, final Document artifactContent, final Document properties, final List<TPropertyConstraint> propertyConstraints, final List<AbstractArtifact> artifacts, final List<String> requiredFeatures) {
		...
		final String artifactId = artifactType + artifactName + contextFile + envFileContent + serviceName + containerPort + endpointPath + endpointKind + soapPortType;
		...
		try {
			...
			final String contextPath = IAEnginePluginDockerComposeServiceImpl.TEMP_DIR + "/docker-compose-ia-" + java.util.UUID.randomUUID().toString();
			...
			if (artifactType.equals("any2api")) {
				...
				final String any2apiExecutablePath = IAEnginePluginDockerComposeServiceImpl.TEMP_DIR + "/any2api-executable-" + java.util.UUID.randomUUID().toString();
			...
			final String publicPort = IAEnginePluginDockerComposeServiceImpl.dcPort(contextPath, serviceName, containerPort);
	...
	@Override
	public boolean undeployImplementationArtifact(final String iaName, final QName nodeTypeImpl, final CSARID csarId, final URI endpointUri) {
	...
	private static void log(final String[] cmd, final String cwd, final String exitCode, final String stdout, final String stderr) {
		final String message = "COMMAND " + java.util.Arrays.deepToString(cmd) + " (CWD " + cwd + ") EXIT " + exitCode + ". STDOUT: " + stdout + ". STDERR: " + stderr;
	...
	private static String diIp(final String containerId) {
		try {
			...
			final String[] cmd = {IAEnginePluginDockerComposeServiceImpl.DOCKER, "inspect", "--format", "{{(index .NetworkSettings.Networks \"" + IAEnginePluginDockerComposeServiceImpl.SHARED_NETWORK + "\").IPAddress}}", containerId};
	...
	private static void dnConnect(final String networkName, final String containerId) throws Exception {
		final String[] cmd = {IAEnginePluginDockerComposeServiceImpl.DOCKER, "network", "connect", networkName, containerId};
		final String[] res = IAEnginePluginDockerComposeServiceImpl.execCmd(cmd);
	}
	
	private static String dcPort(final String contextPath, final String serviceName, final String containerPort) {
		try {
			final String[] cmd = {IAEnginePluginDockerComposeServiceImpl.DOCKER_COMPOSE, "port", serviceName, containerPort};
			final String[] res = IAEnginePluginDockerComposeServiceImpl.execCmd(cmd, contextPath);
	...
	private static void dcDown(final String contextPath) throws Exception {
		final String[] cmd = {IAEnginePluginDockerComposeServiceImpl.DOCKER_COMPOSE, "down", "--rmi", "all", "-v", "--remove-orphans"};
		IAEnginePluginDockerComposeServiceImpl.execCmd(cmd, contextPath);
	}
	
	private static void any2apiGen(final String apispecPath, final String outputPath, final String endpointKind) throws Exception {
		final String[] cmd = {IAEnginePluginDockerComposeServiceImpl.DOCKER, "run", "--rm", "-v", IAEnginePluginDockerComposeServiceImpl.TEMP_DIR + ":" + IAEnginePluginDockerComposeServiceImpl.TEMP_DIR, "any2api/cli:legacy", "-i", endpointKind, "-c", "-o", outputPath, "gen", apispecPath};
