...
public class IAEngineServiceImpl implements IIAEngineService {
	...
	private void deployServiceTemplate(final CSARID csarID, final QName serviceTemplateID) {
		...
		final List<QName> nodeTypeIDs = this.toscaEngineService.getReferencedNodeTypesOfAServiceTemplate(csarID, serviceTemplateID);
	...
	private void deployNodeType(final CSARID csarID, final QName nodeTypeID) {
		...
		for (final QName nodeTypeHierarchyMember : this.toscaEngineService.getNodeTypeHierachy(csarID, nodeTypeID)) {
			...
			final List<QName> nodeTypeImplementationIDs = this.toscaEngineService.getNodeTypeImplementationsOfNodeType(csarID, nodeTypeHierarchyMember);
	...
	private void deployNodeTypeImplementation(final CSARID csarID, final QName nodeTypeImplementationID) {
		...
		for (final QName nodeTypeImplHierarchyMember : this.toscaEngineService.getNodeTypeImplementationTypeHierarchy(csarID, nodeTypeImplementationID)) {
			
			List<String> requiredFeatures = this.toscaEngineService.getRequiredContainerFeaturesOfANodeTypeImplementation(csarID, nodeTypeImplHierarchyMember);
			
			requiredFeatures = IAEngineCapabilityChecker.removeConAndPlanCaps(this.capabilityService, requiredFeatures);
			
			final List<String> implementationArtifactNames = this.toscaEngineService.getImplementationArtifactNamesOfNodeTypeImplementation(csarID, nodeTypeImplHierarchyMember);
			
			for (final String implementationArtifactName : implementationArtifactNames) {
				this.deployImplementationArtifact(csarID, nodeTypeImplHierarchyMember, implementationArtifactName, requiredFeatures);
			}
		}
	 ...
	 */
	private void deployImplementationArtifact(final CSARID csarID, final QName nodeTypeImplementationID, final String implementationArtifactName, final List<String> requiredFeatures) {
		...
		final QName artifactType = this.toscaEngineService.getArtifactTypeOfAImplementationArtifactOfANodeTypeImplementation(csarID, nodeTypeImplementationID, implementationArtifactName);
		final QName artifactRef = this.toscaEngineService.getArtifactTemplateOfAImplementationArtifactOfANodeTypeImplementation(csarID, nodeTypeImplementationID, implementationArtifactName);
		final Document artifactSpecificContent = this.toscaEngineService.getArtifactSpecificContentOfAImplementationArtifactOfANodeTypeImplementation(csarID, nodeTypeImplementationID, implementationArtifactName);
		...
		final List<WSDLEndpoint> endpoints = this.endpointService.getWSDLEndpointsForNTImplAndIAName(nodeTypeImplementationID, implementationArtifactName);
		...
		if ((endpoints != null) && (endpoints.size() > 0)) {
			...
			IAEngineServiceImpl.LOG.debug("ImplementationArtifact: {} of NodeTypeImplementation: {} of CSAR: " + csarID.getFileName() + " is already deployed!", implementationArtifactName, nodeTypeImplementationID);
			
			serviceURI = endpoints.get(0).getURI();
			
		} else {
			serviceURI = this.deployThroughPlugin(csarID, nodeTypeImplementationID, artifactType, artifactSpecificContent, properties, propertyConstraints, artifacts, requiredFeatures);
		}
		...
		if (serviceURI != null) {
			...
			final WSDLEndpoint endpoint = new WSDLEndpoint(serviceURI, portType, csarID, null, nodeTypeImplementationID, implementationArtifactName);
	 ...
	 */
	private URI deployThroughPlugin(final CSARID csarID, final QName nodeTypeImplementationID, final QName artifactType, final Document artifactSpecificContent, final Document properties, final List<TPropertyConstraint> propertyConstraints, final List<AbstractArtifact> artifacts, final List<String> requiredFeatures) {
		...
		if (plugin != null) {
			...
			if (IAEngineCapabilityChecker.capabilitiesAreMet(requiredFeatures, plugin)) {
				...
				serviceEndpoint = plugin.deployImplementationArtifact(csarID, nodeTypeImplementationID, artifactType, artifactSpecificContent, properties, propertyConstraints, artifacts, requiredFeatures);
	...
	public boolean undeployImplementationArtifacts(final CSARID csarID) {
		...
		for (final WSDLEndpoint endpoint : csarEndpoints) {
			...
			if (iaName != null) {
				...
				final List<WSDLEndpoint> endpoints = this.endpointService.getWSDLEndpointsForNTImplAndIAName(nodeTypeImpl, iaName);
				...
				if ((endpoints != null) && (endpoints.size() > 1)) {
					...
					IAEngineServiceImpl.LOG.debug("IA: {} was not undeployed because it is used in other CSARs too. Only its DB entry was removed.", iaName);
					...
					if (iaName != null) {
						...
						final QName artifactType = this.toscaEngineService.getArtifactTypeOfAImplementationArtifactOfANodeTypeImplementation(csarID, nodeTypeImpl, iaName);
						...
						if (plugin != null) {
							...
							final boolean wasUndeployed = plugin.undeployImplementationArtifact(iaName, nodeTypeImpl, csarID, path);
							...
							IAEngineServiceImpl.LOG.warn("No matching plugin found for ArtifactType: {}!", artifactType);
							allUndeployed = false;
						}
					}
				}
