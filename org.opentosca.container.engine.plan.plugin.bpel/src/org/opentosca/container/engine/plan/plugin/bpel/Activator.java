...
public class Activator implements BundleActivator {
	...
	public void start(final BundleContext bundleContext) throws Exception {
		...
		String url = context.getProperty("org.opentosca.container.engine.plan.plugin.bpel.url");

		if (url != null) {
			Settings.setSetting("org.opentosca.container.engine.plan.plugin.bpel.url", url);
		}

		String servicesUrl = context.getProperty("org.opentosca.container.engine.plan.plugin.bpel.services.url");

		if (servicesUrl != null) {
			Settings.setSetting("org.opentosca.container.engine.plan.plugin.bpel.services.url", servicesUrl);
		}

		String userName = context.getProperty("org.opentosca.container.engine.plan.plugin.bpel.username");

		if (userName != null) {
			Settings.setSetting("org.opentosca.container.engine.plan.plugin.bpel.username", userName);
		}

		String password = context.getProperty("org.opentosca.container.engine.plan.plugin.bpel.password");
