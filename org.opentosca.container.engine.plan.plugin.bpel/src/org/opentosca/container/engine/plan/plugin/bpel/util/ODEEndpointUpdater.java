...
public class ODEEndpointUpdater {
	 ...
	 *
	 * @param processFiles
	 *            a list of files containing the complete content of a Apache ODE
	 *            WS-BPEL 2.0 zip file
	 * @param csarId
	 *            the identifier of the CSAR where this process/plan is declared
	 * @return true if every WSDL file used by the process was updated (if needed)
	 *         with endpoints from the openTOSCA Core, else false
	 */
	public boolean changeEndpoints(final List<File> processFiles, final CSARID csarId) {
		this.csarId = csarId;

		Map<QName, List<File>> unchangedFiles = null;
		final File deployXml = this.getDeployXML(processFiles);
		...
		// plan
		try {
			final List<QName> portsInDeployXml = this.getInvokedDeployXMLPorts(deployXml);
	 ...
	 * @param files
	 *            a list of files
	 * @return a file object of a deploy.xml (can be invalid) file if it was found
	 *         in the given list, else null
	 */
	...
	/**
	 * Returns a list of QName's which are referenced in the ODE deploy.xml File as
	 * invoked service.<br>
	 *
	 * @param deployXML
	...
	/**
	 * Returns a List of Services referenced in the provide elements of the given
	 * deploy.xml file
	 *
	 * @param deployXML
	 ...
	 * QNames are taken out of the given list of portTypes and the files from the
	 * other given List
	 *
	 * @param ports
	 ...
	 * @return a Map<QName, List<File>> containing information which porttype is in
	 *         which wsdl file
	 * @throws WSDLException
	 */
	 ...
	 * @return a QName representing the PortType implemented by the given Port if it
	 *         was found inside the WSDL Definition, else null
	 */
	...
	/**
	 * Updates the addresses in the given WSDL files by using endpoints added in the
	 * endpoint db
	 *
	 * @param map
	 *            a map containing <QName,List<File>> pairs. A QName here represents
	 *            a portType that is inside the files
		...
		for (final Object o : wsdlDef.getAllServices().values()) {
			...
			for (final Object obj : service.getPorts().values()) {
				...
				if (port.getBinding().getPortType().getQName().equals(portType)) {
					if (this.changePortAddressWithBpelEngineEndpoints(service, port)) {
						changed = true;
					}
				}
			}
	...
	/**
	 * Updates the addresses inside the given WSDL file by using endpoints inside
	 * the endpoint db
	 *
	 * @param portType
	...
	private boolean changePortAddressWithBpelEngineEndpoints(final Service service, final Port port) {
		...
		for (final Object obj : port.getExtensibilityElements()) {
			...
			for (final WSDLEndpoint endpoint : this.getWSDLEndpointForBpelEngineCallback(service, port)) {
	...
	private boolean changePortAddressWithEndpointDB(final Port port) {
		...
		for (final Object obj : port.getExtensibilityElements()) {
			...
			final ExtensibilityElement element = (ExtensibilityElement) obj;
			for (final WSDLEndpoint endpoint : this.getWSDLEndpointsFromEndpointDB(port)) {
	 ...
	 * @return a list containing all WSDLEndpoints that matches the portTypes of the
	 *         given Port
	 */
	private List<WSDLEndpoint> getWSDLEndpointsFromEndpointDB(final Port port) {
		...
		if (ODEEndpointUpdater.endpointService != null) {
			...
			final List<WSDLEndpoint> temp = ODEEndpointUpdater.endpointService
	...
	/**
	 * Changes the address in the given ExtensibilityElement to address given in the
	 * given WSDLEndpoint
	 *
	 * @param element
	 ...
	 * @return true if changing was successful, this means the ExtensibilityElement
	 *         had the type
