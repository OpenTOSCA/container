...
public class ODEEndpointUpdater {
	...
	public boolean changeEndpoints(final List<File> processFiles, final CSARID csarId) {
		...
		try {
			...
			final Map<QName, List<File>> changeMap = this.getWSDLtoChange(portsInDeployXml, this.getAllWSDLFiles(processFiles));
		...
		for (final QName portType : unchangedFiles.keySet()) {
			...
			for (final File file : unchangedFiles.get(portType)) {
				ODEEndpointUpdater.LOG.warn("WSDL file {} which contained portType {} and couldn't be updated", file.toPath().toString(), portType.toString());
			}
		}
	...
	private boolean changePortAddressWithBPSEndpoints(final Service service, final Port port) {
		boolean changed = false;
		
		for (final Object obj : port.getExtensibilityElements()) {
			final ExtensibilityElement element = (ExtensibilityElement) obj;
			for (final WSDLEndpoint endpoint : this.getWSDLEndpointForBPSCallback(service, port)) {
	...
	private List<WSDLEndpoint> getWSDLEndpointForBPSCallback(final Service service, final Port port) {
		...
		try {
			endpoints.add(new WSDLEndpoint(new URI(callbackEndpoint), port.getBinding().getPortType().getQName(), null, null, null, null));
	...
	private List<WSDLEndpoint> getWSDLEndpointsFromEndpointDB(final Port port) {
		...
		if (ODEEndpointUpdater.endpointService != null) {
			...
			final List<WSDLEndpoint> temp = ODEEndpointUpdater.endpointService.getWSDLEndpoints(port.getBinding().getPortType().getQName(), this.csarId);
