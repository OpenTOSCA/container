...
public class CopyOfIAEnginePluginWarTomcatServiceImpl {
	...
	public void deployImplementationArtifact(final CSARID csarID, final File warFile) {
		...
		if ((warFile != null) && warFile.exists()) {
			...
			if (deployPath != null) {
				...
				CopyOfIAEnginePluginWarTomcatServiceImpl.LOG.info("Complete war deployment of {} to the path {}", warFile.getName(), deployPath);
			}
	...
	private String deploy(final CSARID csarID, final File warFile) {
		...
		if (this.isRunning()) {
			...
			try {
				...
				final String uri = CopyOfIAEnginePluginWarTomcatServiceImpl.URL + "/manager/text/deploy?update=true&path=" + deployPath;
				...
				final HttpResponse httpResponse = this.httpService.Put(uri, uploadEntity, CopyOfIAEnginePluginWarTomcatServiceImpl.USERNAME, CopyOfIAEnginePluginWarTomcatServiceImpl.PASSWORD);
				...
				CopyOfIAEnginePluginWarTomcatServiceImpl.LOG.error("UnsupportedEncodingException occured while deploying the WAR-File: {}!", fileName, e);
			} catch (final ClientProtocolException e) {
				CopyOfIAEnginePluginWarTomcatServiceImpl.LOG.error("ClientProtocolException occured while deploying the WAR-File: {}!", fileName, e);
			} catch (final IOException e) {
				CopyOfIAEnginePluginWarTomcatServiceImpl.LOG.error("IOException occured while deploying the WAR-File: {}!", fileName, e);
			}

		} else {
			CopyOfIAEnginePluginWarTomcatServiceImpl.LOG.error("Tomcat isn't running or can't be accessed! Can't deploy {}!", fileName);
		}
			...
			for (final AbstractArtifact artifact : artifacts) {
				...
				for (final AbstractFile file : files) {
					...
					if (this.isADeployableWar(file)) {
						CopyOfIAEnginePluginWarTomcatServiceImpl.LOG.info("Deployable WAR-File with name {} found.", file.getName());
	...
	private boolean isADeployableWar(final AbstractFile file) {
		...
		if (file.getName().toLowerCase().endsWith(".war")) {
			...
			CopyOfIAEnginePluginWarTomcatServiceImpl.LOG.warn("Although the plugin-type and the IA-type are matching, the file {} can't be un-/deployed from this plugin.", file.getName());
		}
		...
		if (properties != null) {
			...
			for (int i = 0; i < list.getLength(); i++) {
				...
				if (this.containsEndpointSuffix(propNode)) {
					...
					CopyOfIAEnginePluginWarTomcatServiceImpl.LOG.info("ServiceEndpointSuffix found: {}", endpointSuffix);
	...
	private URI getURI(final String endpoint) {
		...
		if (endpoint != null) {
			try {
				...
				CopyOfIAEnginePluginWarTomcatServiceImpl.LOG.error("URISyntaxException occurred while creating endpoint URI: {} ", endpoint, e);
			}
		}
		return endpointURI;
	}
	...
	private boolean isRunning() {
		...
		CopyOfIAEnginePluginWarTomcatServiceImpl.LOG.info("Checking if Tomcat is running on '" + CopyOfIAEnginePluginWarTomcatServiceImpl.URL + "' and can be accessed with \"" + USERNAME + "\" and \"" + PASSWORD + "\"");
		...
		try {
			httpResponse = this.httpService.Get(url, CopyOfIAEnginePluginWarTomcatServiceImpl.USERNAME, CopyOfIAEnginePluginWarTomcatServiceImpl.PASSWORD);
	...
	public List<String> getSupportedTypes() {
		...
		CopyOfIAEnginePluginWarTomcatServiceImpl.LOG.debug("Getting Types: {}.", CopyOfIAEnginePluginWarTomcatServiceImpl.TYPES);
	...
	public List<String> getCapabilties() {
		...
		CopyOfIAEnginePluginWarTomcatServiceImpl.LOG.debug("Getting Plugin-Capabilities: {}.", CopyOfIAEnginePluginWarTomcatServiceImpl.CAPABILITIES);
