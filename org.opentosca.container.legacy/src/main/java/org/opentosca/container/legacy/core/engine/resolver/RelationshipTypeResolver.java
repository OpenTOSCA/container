...
/**
 * The RelationshipTypeResolver resolves references inside of TOSCA RelationshipTypes according to
 * the TOSCA specification wd14. Each found element and the document in which the element is nested
 * is stored by the org.opentosca.core.model.toscareferencemapping.ToscaReferenceMapper.
 * <p>
 * Preconditions for resolving a RelationshipType: Definitions has to be valid in all kind of
 * meanings.
 *
...
public class RelationshipTypeResolver extends GenericResolver {
  ...
  private final Logger LOG = LoggerFactory.getLogger(RelationshipTypeResolver.class);


  /**
   * Instantiate an object of the Resolver to resolve references inside of RelationshipTypes. This
   * constructor sets the ReferenceMapper which searches for references.
   *
   * @param referenceMapper
   */
  public RelationshipTypeResolver(final ReferenceMapper referenceMapper) {
    super(referenceMapper);
  }

  /**
   * Resolves all RelationshipTypes inside of a Definitions and stores the mapping into the
  ...
  public boolean resolve(final Definitions definitions) {
    ...
    for (final TExtensibleElements element : definitions.getServiceTemplateOrNodeTypeOrNodeTypeImplementation()) {
      if (element instanceof TRelationshipType) {
        ...
        final TRelationshipType relationshipType = (TRelationshipType) element;

        // store the RelationshipType
        String targetNamespace;
        if (relationshipType.getTargetNamespace() != null
          && !relationshipType.getTargetNamespace().equals("")) {
        ...
        if (relationshipType.getDerivedFrom() != null) {
          errorOccurred = errorOccurred
            || !this.referenceMapper.searchToscaElementByQNameWithName(relationshipType.getDerivedFrom()
              .getTypeRef(),
            ElementNamesEnum.RELATIONSHIP_TYPE);
        }
