...
public class PortabilityServiceImpl implements IPortabilityService {
	...
	@Override
	public Artifacts getNodeTemplateArtifacts(final CSARID csarID, final QName serviceTemplateID, final QName nodeTemplateID, final ArtifactType artifactType, final String deploymentArtifactName, final String interfaceName, final String operationName) {
		
		// retrieve qnames of all nodeTypes => and qnames of implementations of
		// this nodeTypes
		final QName nodeTypeOfNodeTemplate = toscaEngineService.getNodeTypeOfNodeTemplate(csarID, serviceTemplateID, nodeTemplateID.getLocalPart());
		
		// TODO: null check nodeTypeOfNodeTemplate
		final List<QName> nodeTypeImplementationsOfNodeType = toscaEngineService.getNodeTypeImplementationsOfNodeType(csarID, nodeTypeOfNodeTemplate);
		...
		this.fillFilteredArtifactsOfNodeTypeImplByName(filteredDAList, filteredIAList, csarID, nodeTypeImplementationsOfNodeType, deploymentArtifactName, interfaceName, operationName);
		
		this.fillFilteredArtifactsOfNodeTemplateByName(filteredDAList, filteredIAList, csarID, nodeTemplateID, deploymentArtifactName, interfaceName, operationName);
	...
	@Override
	public Artifacts getRelationshipTemplateArtifacts(final CSARID csarID, final QName serviceTemplateID, final QName relationshipTemplateID, final ArtifactType artifactType, final String deplArtifactName, final String interfaceName, final String operationName) {
		
		// retrieve qnames of all nodeTypes => and qnames of implementations of
		// this nodeTypes
		final QName relationshipTypeOfNodeTemplate = toscaEngineService.getRelationshipTypeOfRelationshipTemplate(csarID, serviceTemplateID, relationshipTemplateID.getLocalPart());
		// TODO: null check nodeTypeOfNodeTemplate
		final List<QName> relationshipTypeImplementationsOfNodeType = toscaEngineService.getRelationshipTypeImplementationsOfRelationshipType(csarID, relationshipTypeOfNodeTemplate);
		...
		this.fillFilteredArtifactsOfRelationshipTypeImplByName(filteredDAList, filteredIAList, csarID, relationshipTypeImplementationsOfNodeType, deplArtifactName, interfaceName, operationName);
	 ...
	 *         lists
	 */
	private Artifacts buildTArtifactsResult(final List<ResolvedDeploymentArtifact> filteredDAList, final List<ResolvedImplementationArtifact> filteredIAList, final ArtifactType artifactType) {
		...
		if (ArtifactType.DA.equals(artifactType)) {
			...
			for (final ResolvedDeploymentArtifact da : filteredDAList) {
				...
				if (da.getArtifactSpecificContent() != null) {
					newDA = new DeploymentArtifact(da.getName(), da.getType().toString(), da.getArtifactSpecificContent());
				} else {
		...
		if ((ArtifactType.IA).equals(artifactType)) {
			...
			for (final ResolvedImplementationArtifact ia : filteredIAList) {
				...
				if (ia.getArtifactSpecificContent() != null) {
					newIA = new ImplementationArtifact(ia.getOperationName(), ia.getInterfaceName(), ia.getType().toString(), ia.getArtifactSpecificContent());
				} else {
					// we need to get the references
					newIA = new ImplementationArtifact(ia.getOperationName(), ia.getInterfaceName(), ia.getType().toString(), ia.getReferences());
				}
	 ...
	 *
	 * @param filteredDAList list of resolvedArtifacts where the filtered DAs
	 *            will be STORED!!!
	 * @param filteredIAList list of resolvedArtifacts where the filtered IAs
	 *            will be STORED!!
	 * @param csarID csarID of the CSAR
	 * @param nodeTemplateID NodeTemplate ID
	 * @param deploymentArtifactNameFilter Filter which will be applied
	 *            (.equals) to the resolved DeploymentArtifacts
	 * @param interfaceNameFilter Filter which will be applied (.equals) to the
	 *            resolved ImplArtifacts
	 * @param operationNameFilter Filter which will be applied (.equals) to the
	 *            resolved ImplArtifacts
	 */
	private void fillFilteredArtifactsOfNodeTemplateByName(final List<ResolvedDeploymentArtifact> filteredDAList, final List<ResolvedImplementationArtifact> filteredIAList, final CSARID csarID, final QName nodeTemplateID, final String deploymentArtifactName, final String interfaceName, final String operationName) {

		final ResolvedArtifacts resolvedTemp = toscaEngineService.getResolvedArtifactsOfNodeTemplate(csarID, nodeTemplateID);
		...
		for (final ResolvedDeploymentArtifact resolvedDArtifact : resolvedTemp.getDeploymentArtifacts()) {
			if ((null == deploymentArtifactName) || deploymentArtifactName.equals("") || deploymentArtifactName.equals(resolvedDArtifact.getName())) {
	 ...
	 *            will be STORED!!
	 * @param csarID csarID of the CSAR
	 * @param nodeTypeImplementations List of nodeTypeImplementations for which
	 ...
	 *            resolved ImplArtifacts
	 */
	private void fillFilteredArtifactsOfNodeTypeImplByName(final List<ResolvedDeploymentArtifact> filteredDAList, final List<ResolvedImplementationArtifact> filteredIAList, final CSARID csarID, final List<QName> nodeTypeImplementations, final String deploymentArtifactNameFilter, final String interfaceNameFilter, final String operationNameFilter) {
		...
		for (final QName ntImplQName : nodeTypeImplementations) {
			...
			final ResolvedArtifacts resolvedTemp = toscaEngineService.getResolvedArtifactsOfNodeTypeImplementation(csarID, ntImplQName);
			...
			// specified or matching)
			for (final ResolvedDeploymentArtifact resolvedDArtifact : resolvedTemp.getDeploymentArtifacts()) {
				if ((null == deploymentArtifactNameFilter) || deploymentArtifactNameFilter.equals("") || deploymentArtifactNameFilter.equals(resolvedDArtifact.getName())) {
					filteredDAList.add(resolvedDArtifact);
				}
			}
	 ...
	 *            will be STORED!!
	 * @param csarID csarID of the CSAR
	 * @param relationshipTypeImplementations List of
	 *            relationshipTypeImplementations for which the Artifacts will
	 *            be resolved
	 * @param artifactNameFilter Filter which will be applied (.equals) to the
	 *            resolved Artifacts
	 */
	private void fillFilteredArtifactsOfRelationshipTypeImplByName(final List<ResolvedDeploymentArtifact> filteredDAList, final List<ResolvedImplementationArtifact> filteredIAList, final CSARID csarID, final List<QName> relationshipTypeImplementations, final String deploymentArtifactNameFilter, final String interfaceNameFilter, final String operationNameFilter) {
		for (final QName ntImplQName : relationshipTypeImplementations) {
			// add the list of ImplementationArtifactNames of the
			// NodeTypeImplementation to the iaNames-List
			// if name is specified only add matching artifactNames
			
			final ResolvedArtifacts resolvedTemp = toscaEngineService.getResolvedArtifactsOfRelationshipTypeImplementation(csarID, ntImplQName);
			...
			// specified or matching)
			for (final ResolvedDeploymentArtifact resolvedDArtifact : resolvedTemp.getDeploymentArtifacts()) {
				if ((deploymentArtifactNameFilter == null) || deploymentArtifactNameFilter.equals(resolvedDArtifact.getName())) {
					filteredDAList.add(resolvedDArtifact);
				}
			}
