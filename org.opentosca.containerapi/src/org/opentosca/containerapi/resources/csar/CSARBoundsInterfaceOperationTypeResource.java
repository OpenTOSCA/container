...
public class CSARBoundsInterfaceOperationTypeResource {

    private static final Logger LOG = LoggerFactory.getLogger(CSARBoundsInterfaceOperationTypeResource.class);
    CSARID csarID;
    String intName;
    String opName;

    public CSARBoundsInterfaceOperationTypeResource(CSARID csarID, String intName, String op) {
	...
	refs.getReference().add(new Reference(Utilities.buildURI(uriInfo.getAbsolutePath().toString(), "NodeOperation"),
	    XLinkConstants.SIMPLE, "NodeOperation"));
	refs.getReference()
	.add(new Reference(Utilities.buildURI(uriInfo.getAbsolutePath().toString(), "RelationshipOperation"),
	    XLinkConstants.SIMPLE, "RelationshipOperation"));
	refs.getReference().add(new Reference(Utilities.buildURI(uriInfo.getAbsolutePath().toString(), "Plan"),
    ...
    @Path("NodeOperation")
    @Produces(ResourceConstants.LINKED_XML)
    public Response getNodeOperation(@Context UriInfo uriInfo) {
    ...
    @Produces(ResourceConstants.LINKED_XML)
    public Response getRelationshipOperation(@Context UriInfo uriInfo) {
     ...
     */
    @Path("Plan")
    public CSARBoundsInterfaceOperationTypePlanResource getPlan(@Context UriInfo uriInfo) {
	return new CSARBoundsInterfaceOperationTypePlanResource(csarID, intName, opName);
    }
