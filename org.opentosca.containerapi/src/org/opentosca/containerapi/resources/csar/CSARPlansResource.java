...
public class CSARPlansResource {
	...
	public Response getReferences(@Context UriInfo uriInfo) {
		...
		Map<PlanTypes, LinkedHashMap<QName, TPlan>> linkedMapOfPublicPlans = ToscaServiceHandler.getToscaEngineService().getToscaReferenceMapper().getCSARIDToPlans(csarID);
		...
		for (PlanTypes type : PlanTypes.values()) {
			for (QName planId : linkedMapOfPublicPlans.get(type).keySet()) {
				...
				refs.getReference().add(new Reference(Utilities.buildURI(uriInfo.getAbsolutePath().toString(), planId.getLocalPart()), XLinkConstants.SIMPLE, planId.getLocalPart()));
			}
		}
	...
	public TPlanDTO getPublicPlan(String planName){
		...
		if (null == map) {
			CSARPlansResource.LOG.error("For CSARID \"" + csarID.toString() + "\" there is no stored informations about plans.");
			return null;
		}
		...
		CSARPlansResource.LOG.debug("Return the plan with name " + planName + "\" for CSAR \"" + csarID.toString() + "\".");
