...
public class ServiceTemplateInstancesResource {
	...
	public References getRefs(UriInfo uriInfo, String buildPlanCorrId) {
		...
		if (null == buildPlanCorrId || buildPlanCorrId.equals("")
			...
			List<ServiceInstance> serviceInstances = service.getServiceInstancesWithDetails(csarId, serviceTemplateID,
					null);
			...
			for (ServiceInstance serviceInstance : serviceInstances) {
				...
				log.debug("ST ID of service \"{}\":\"{}\" vs. path \"{}\":\"{}\"",
						serviceInstance.getServiceTemplateID().getNamespaceURI(),
						serviceInstance.getServiceTemplateID().getLocalPart(), serviceTemplateID.getNamespaceURI(),
				...
				if (serviceInstance.getServiceTemplateID().equals(serviceTemplateID)) {
					...
					refs.getReference()
							.add(new Reference(
									Utilities.buildURI(uriInfo.getAbsolutePath().toString(),
		...
		// get instance id of plan correlation only
			...
			int instanceId = BuildCorrelationToInstanceMapping.instance
					.getServiceTemplateInstanceIdForBuildPlanCorrelation(buildPlanCorrId);
			refs.getReference()
					.add(new Reference(
							Utilities.buildURI(uriInfo.getAbsolutePath().toString(), Integer.toString(instanceId)),
							XLinkConstants.SIMPLE, Integer.toString(instanceId)));

			log.debug("Returning only the Service Template instance ID for correlation {} ({}).", buildPlanCorrId,
					instanceId);
	...
	public Response createServiceInstance(@Context UriInfo uriInfo, String xml) {
		...
		try {
			...
			PlanInvocationEngineHandler.planInvocationEngine.correctCorrelationToServiceTemplateInstanceIdMapping(
					csarId, serviceTemplateID, corr, serviceTemplateInstanceId);
	...
	private String postManagementPlanJSON(UriInfo uriInfo, String json) throws UnsupportedEncodingException {
		...
		while (iterator.hasNext()) {
			...
			para.setName(JSONUtils
					.withoutQuotationMarks(tmp.get("InputParameter").getAsJsonObject().get("Name").toString()));
			para.setRequired(TBoolean.fromValue(JSONUtils
					.withoutQuotationMarks(tmp.get("InputParameter").getAsJsonObject().get("Required").toString())));
			para.setType(JSONUtils
					.withoutQuotationMarks(tmp.get("InputParameter").getAsJsonObject().get("Type").toString()));
			// if a parameter value is not set, just add "" as value
			if (null != tmp.get("InputParameter").getAsJsonObject().get("Value")) {
				para.setValue(JSONUtils
						.withoutQuotationMarks(tmp.get("InputParameter").getAsJsonObject().get("Value").toString()));
			} else {
				para.setValue("");
			}
		...
		while (iterator.hasNext()) {
			...
			JsonObject tmp = iterator.next().getAsJsonObject();
			para.setName(JSONUtils
					.withoutQuotationMarks(tmp.get("OutputParameter").getAsJsonObject().get("Name").toString()));
			para.setRequired(TBoolean.fromValue(JSONUtils
					.withoutQuotationMarks(tmp.get("OutputParameter").getAsJsonObject().get("Required").toString())));
			para.setType(JSONUtils
					.withoutQuotationMarks(tmp.get("OutputParameter").getAsJsonObject().get("Type").toString()));
		...
		String correlationID = IOpenToscaControlServiceHandler.getOpenToscaControlService().invokePlanInvocation(csarId,
				serviceTemplateID, -1, plan);

		log.debug("Return correlation ID of running plan: " + correlationID);

		String url = uriInfo.getBaseUri().toString() + "CSARs/" + csarId.getFileName() + "/ServiceTemplates/"
				+ URLEncoder.encode(serviceTemplateID.toString(), "UTF-8") + "/Instances?BuildPlanCorrelationId="
				+ correlationID;
