...
public class RelationshipTemplateInstancePropertiesResource {
	...
	public RelationshipTemplateInstancePropertiesResource(int id) {
	...
	public Document getProperties(List<String> propertiesList) {
		...
		try {
			Document properties = service.getRelationInstanceProperties(IdConverter.relationInstanceIDtoURI(relationInstanceID), qnameList);
			return properties;
	...
	public Response setProperties(@Context UriInfo uriInfo, Document xml) {
		IInstanceDataService service = InstanceDataServiceHandler.getInstanceDataService();
		try {
			service.setRelationInstanceProperties(IdConverter.relationInstanceIDtoURI(relationInstanceID), xml);
		} catch (ReferenceNotFoundException e) {
			throw new GenericRestException(Status.NOT_FOUND, e.getMessage());
		}
		SimpleXLink xLink = new SimpleXLink(uriInfo.getAbsolutePath(), "RelationInstance: " + relationInstanceID + " Properties");
