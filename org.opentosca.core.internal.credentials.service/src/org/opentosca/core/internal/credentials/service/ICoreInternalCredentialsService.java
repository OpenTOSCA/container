package org.opentosca.core.internal.credentials.service;
...
/**
 * 
 * This interface of the Core Internal Credentials Service defines methods for<br />
 * <br />
 * - storing, getting and deleting credentials.<br />
 * - checking if credentials are set in a storage provider.<br />
 * - setting credentials in a storage provider.<br />
 * - getting the name of the credentials identity and key of a storage provider.<br />
 * - getting all available storage providers. <br />
 * <br />
 ...
 * 
 */
public interface ICoreInternalCredentialsService {
	
	/**
	 * Stores credentials {@code credentials}.<br />
	 * Also injects the credentials in the appropriate storage provider if the
	 * following conditions are fulfilled:<br />
	 * - Storage provider is available.<br />
	 * - Storage provider has no injected credentials.<br />
	 * <br />
	 * Note: Credentials will be only stored if storage provider of credentials
	 * needs credentials. If storage provider is not available, we can't check
	 * if this requirement is fulfilled.
	 * 
	 * @param credentials to store.
	 * 
	 * @return Generated ID of stored credentials.
	 * @throws UserException if {@code credentials} contains missing required
	 *             data, are already stored according to unique constraints
	 *             defined in {@link Credentials} or storage provider of
	 ...
	 * @return All stored {@link Credentials} for storage provider
	...
	/**
	 * Sets / injects stored credentials {@code credentialsID} in their storage
	 * provider.
	 * 
	 * @param credentialsID of credentials.
	 * @throws SystemException if storage provider of credentials is not
	 *             available.
	 * @throws UserException if credentials were not found or storage provider
	 *             of credentials {@code credentialsID} needs no credentials.
	 */
	public void setCredentialsInStorageProvider(long credentialsID) throws UserException, SystemException;
	
	/**
	 * Deletes credentials in storage provider {@code storageProviderID}, if
	 * necessary.
	 * 
	 * @param storageProviderID of storage provider.
	...
	/**
	 * @param storageProviderID of storage provider
	 * @return {@code true} if storage provider {@code storageProviderID} has
	 *         currently credentials, otherwise {@code false}.
	 * @throws SystemException if storage provider {@code storageProviderID} is
	 *             not available.
	 */
	...
	/**
	 * @param credentialsID of credentials.
	 * @return {@code true} if storage provider of credentials
	 *         {@code credentialsID} has these credentials, otherwise
	 *         {@code false}.<br />
	 *         Also {@code false} will be returned if storage provider of
	 *         credentials {@code credentialsID} is not available.
	 * 
	...
	/**
	 * @param storageProviderID of storage provider
	 * @return {@code true} if storage provider {@code storageProviderID} needs
	 *         credentials, otherwise {@code false}.
	 * @throws SystemException if storage provider {@code storageProviderID} is
	 *             not available.
	 */
	...
	/**
	 * @param storageProviderID of storage provider
	 * @return Name of the credentials identity of storage provider
	 *         {@code storageProviderID}, e.g. for AWS S3 it's the Access Key
	 *         ID.
	 * @throws SystemException if storage provider {@code storageProviderID} is
	 *             not available.
	 */
	...
	/**
	 * @param storageProviderID of storage provider
	 * @return Name of the credentials key of storage provider
	 *         {@code storageProviderID}, e.g. for AWS S3 it's the Secret Access
	 *         Key.
	 * @throws SystemException if storage provider {@code storageProviderID} is
	 *             not available.
	 */
