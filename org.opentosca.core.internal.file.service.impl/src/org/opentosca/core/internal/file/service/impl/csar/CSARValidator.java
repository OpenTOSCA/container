...
/**
 * Validates the content of a CSAR.<br />
 * <br />
...
public class CSARValidator {
	...
	/**
	 * Contains all directories and files in the unpack directory of the CSAR to
	 * validate.
	 */
	 ...
	 * After creation {@link #isValid()} should be called to validate the CSAR
	 * content.
	 * 
	 * @param csarID of CSAR
	 * @param csarUnpackDir - absolute path of CSAR unpack directory
	 * @param csarVisitor - must contain all files and directories in
	 *            {@code csarUnpackDir}.
	 */
	public CSARValidator(CSARID csarID, Path csarUnpackDir, DirectoryVisitor csarVisitor) {
		this.CSAR_ID = csarID;
	...
	public boolean isValid() {
		...
		Set<Path> csarFiles = this.CSAR_VISITOR.getVisitedFiles();
	...
	/**
	 * 
	 * @return Occurred errors during validation of CSAR. If CSAR was not
	 *         validated yet using {@link CSARValidator#isValid()} or no errors
	 *         occurred {@code null}.
	 */
	 ...
	 *         {@code csarID} contains at least one TOSCA file, otherwise
	 *         {@code false}.
	 */
	private boolean containsTOSCAs(CSARID csarID, Path csarUnpackDir, Set<Path> csarFiles) {
		
		Path csarDefinitionsDirAbsPath = csarUnpackDir.resolve(this.CSAR_DEFINITIONS_DIR_REL_PATH);
		
		for (Path csarFile : csarFiles) {
			if (csarFile.startsWith(csarDefinitionsDirAbsPath) /*
																 ...
																 * )
																 */) {
				CSARValidator.LOG.debug("At least one file was found in directory \"{}\" of CSAR \"{}\".", this.CSAR_DEFINITIONS_DIR_REL_PATH, this.CSAR_ID);
				return true;
			}
		}
		
		this.errorMessage.append("\n");
		this.errorMessage.append("No files were found in directory \"" + this.CSAR_DEFINITIONS_DIR_REL_PATH + "\" of CSAR \"" + this.CSAR_ID + "\". There must be at least one!");
	 ...
	 * @param csarUnpackDir - absolute path of CSAR unpack directory
	 * @param csarFiles - all files in {@code csarUnpackDir}.
	 * @return {@code true} if TOSCA meta file exists in CSAR {@code csarID},
	 *         otherwise {@code false}.
	 */
	private boolean existsTOSCAMetaFile(CSARID csarID, Path csarUnpackDir, Set<Path> csarFiles) {
		
		Path toscaMetaFileAbsPath = csarUnpackDir.resolve(this.TOSCA_META_FILE_REL_PATH);
		
		if (csarFiles.contains(toscaMetaFileAbsPath)) {
			CSARValidator.LOG.debug("TOSCA meta file exists at \"{}\" in CSAR \"{}\".", this.TOSCA_META_FILE_REL_PATH, this.CSAR_ID);
			return true;
		}
		
		this.errorMessage.append("\n");
		this.errorMessage.append("TOSCA meta file does not exist at \"" + this.TOSCA_META_FILE_REL_PATH + "\" in CSAR \"" + this.CSAR_ID + "\".");
