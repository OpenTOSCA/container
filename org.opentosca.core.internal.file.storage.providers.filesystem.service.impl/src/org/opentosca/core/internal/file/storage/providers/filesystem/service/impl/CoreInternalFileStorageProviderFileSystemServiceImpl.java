package org.opentosca.core.internal.file.storage.providers.filesystem.service.impl;
...
/**
 * File system storage provider.<br />
 * It's implemented by extending from {@link AbstractJCloudsFileStorageProvider}
 * , so it uses jclouds to access the file system. This class primarily contains
 * meta data of the storage provider and some necessary jclouds configuration
 * like the location of the file system-based blobstore.<br />
 * <br />
 * A CSAR file will be stored at the following path:<br />
 * {@code <BlobStorePath>/<containerName>/<csarID>/<relPathOfFileToCSARRoot>} <br />
 * <br />
 ...
 * or getting the current one is possible via OSGi console commands.<br />
 * <br />
...
public class CoreInternalFileStorageProviderFileSystemServiceImpl extends AbstractJCloudsFileStorageProvider implements ICoreInternalFileStorageProviderService, CommandProvider {
	...
	protected Properties overwriteJCloudsProperties() {
		Properties props = super.overwriteJCloudsProperties();
	...
	public void _setFilesystemContainerName(CommandInterpreter ci) {
	...
	public void _printFilesystemContainerName(CommandInterpreter ci) {
