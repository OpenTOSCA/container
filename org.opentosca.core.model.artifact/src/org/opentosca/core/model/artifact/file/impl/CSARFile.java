package org.opentosca.core.model.artifact.file.impl;
 ...
 * provider.<br />
 * Provides methods to get it's meta data using the local stored meta data and
 * fetch the file from the appropriate storage provider.<br />
 * <br />
...
public class CSARFile extends AbstractFile {
	 ...
	 * @see ICoreInternalFileStorageProviderService
	 */
	public CSARFile(String relFilePathToCSARRoot, CSARID csarID, String storageProviderID) {
	 ...
	 */
	private boolean isStorageProviderReady(String storageProviderID) {
		
		CSARFile.LOG.debug("Checking if storage provider \"{}\" is ready...", storageProviderID);
		
		ICoreInternalFileStorageProviderService storageProvider = CSARFile.STORAGE_PROVIDERS.get(storageProviderID);
	...
	public Path getFile() throws SystemException {
		...
		if (this.isStorageProviderReady(this.STORAGE_PROVIDER_ID)) {
			...
			ICoreInternalFileStorageProviderService storageProvider = CSARFile.STORAGE_PROVIDERS.get(this.STORAGE_PROVIDER_ID);
			
			Path targetFile = CSARFile.fileAccessService.getTemp().toPath().resolve(this.getName());
			
			String relFilePathOnStorageProvider = this.buildFileOfCSARStorageProviderPath(this.CSAR_ID, this.getPath());
	 ...
	 */
	private String buildFileOfCSARStorageProviderPath(CSARID csarID, String relFilePathToCSARRoot) {
	...
	public boolean equals(Object file) {
		if (file instanceof CSARFile) {
			CSARFile csarFile = (CSARFile) file;
