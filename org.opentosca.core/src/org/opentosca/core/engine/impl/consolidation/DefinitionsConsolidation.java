...
public class DefinitionsConsolidation {
	...
	public boolean consolidateCSAR(final CSARID csarID) {
		...
		if (!ret) {
			this.LOG.error("Consolidation of the exported interfaces of CSAR \"" + csarID + "\" produced one or more errors.");
		}
		ret = ret && this.policyConsolidation.consolidate(csarID);
		if (!ret) {
			this.LOG.error("Consolidation of the Policies inside the CSAR \"" + csarID + "\" produced one or more errors.");
		}
