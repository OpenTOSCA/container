package org.opentosca.core.engine.impl.consolidation;
...
import org.opentosca.core.engine.impl.ToscaEngineServiceImpl;
import org.opentosca.core.engine.impl.ToscaReferenceMapper;
...
import org.opentosca.core.tosca.model.TExportedOperation.Plan;
...
public class ExportedInterfacesConsolidation {
			...
			for (final TExportedInterface iface : this.toscaReferenceMapper.getExportedInterfacesOfCSAR(csarID).get(serviceTemplateID)) {
				...
				for (final TExportedOperation operation : iface.getOperation()) {
					...
					if (null != planReference) {
						...
						this.toscaReferenceMapper.storePlanInputMessageID(csarID, planID, this.getInputMessageQName(csarID, iface.getName(), operation.getName(), toscaPlan.getId(), toscaPlan, this.toscaReferenceMapper.getListOfWSDLForCSAR(csarID)));
	 ...
	 */
	private QName getInputMessageQName(final CSARID csarID, final String wsdlInterfaceName, final String wsdlOperationName, final String planID, final TPlan boundaryPlan, final List<Document> list) {

		this.LOG.debug("Try to find the InputMessageID for CSAR " + csarID + " and plan " + boundaryPlan.getId());

		this.LOG.debug("countwsdl: " + list.size() + " interfacename:" + wsdlInterfaceName + " operationname:" + wsdlOperationName);
			...
			try {
				...
				if (messageQName.getLength() == 1) {
					...
					if (messages.getLength() == 1) {
						...
						final QName messageID = new QName(doc.lookupNamespaceURI(value.substring(0, value.indexOf(":"))), value.substring(value.indexOf(":") + 1));
