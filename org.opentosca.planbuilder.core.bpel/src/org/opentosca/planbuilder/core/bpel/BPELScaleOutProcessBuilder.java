...
public class BPELScaleOutProcessBuilder extends AbstractScaleOutPlanBuilder {
	...
	private void addRecursiveInstanceSelection(BPELPlan plan, PropertyMap map, AbstractNodeTemplate nodeTemplate) {
		...
		BPELPlanContext nodeContext = this.createContext(nodeTemplate, plan, map);
		...
		BPELPlanContext relationContext = this.createContext(relationshipTemplate, plan, map);
		...
		String responseVarName = "recursiveSelection_NodeInstance_" + nodeTemplate.getId() + System.currentTimeMillis()
				+ "_Response";
		nodeContext.addVariable(responseVarName, BPELPlan.VariableType.TYPE, anyTypeDeclId);

		// fetch relationInstance data
		try {
			Node fetchRelationInstanceData = new BPELProcessFragments()
					.createRESTDeleteOnURLBPELVarAsNode(relationInstanceVarName, responseVarName);
		...
		try {
			Node queryNodeInstanceUrl = new BPELProcessFragments().createAssignXpathQueryToStringVarFragmentAsNode(
					"recursiveSelection_fetchNodeInstance" + System.currentTimeMillis(), xpathQuery,
					nodeInstanceVarName);
	...
	private void addRecursiveInstanceSelection(BPELPlan plan, PropertyMap map,
			AbstractRelationshipTemplate relationshipTemplate) {
		// fetch relationInstance variable of relationship template
		BPELPlanContext relationContext = this.createContext(relationshipTemplate, plan, map);
		String relationshipTemplateInstanceVarName = this.findInstanceVar(relationContext, relationshipTemplate.getId(),
				false);

		// fetch nodeInstance variable of source node template
		BPELPlanContext nodeContext = this.createContext(relationshipTemplate.getTarget(), plan, map);
		String nodeTemplateInstanceVarName = this.findInstanceVar(nodeContext, relationshipTemplate.getTarget().getId(),
				true);
		...
		QName stringTypeDeclId = relationContext
				.importQName(new QName("http://www.w3.org/2001/XMLSchema", "string", "xsd"));
		String requestVarName = "recursiveSelection_RelationInstance_" + relationshipTemplate.getId()
				+ System.currentTimeMillis() + "_Request";
		String responseVarName = "recursiveSelection_RelationInstance_" + relationshipTemplate.getId()
		...
		try {
			Node requestRelationInstance = new BPELProcessFragments()
					.createBPEL4RESTLightRelationInstancesTargetNodeInstanceQueryGETAsNode(serviceInstanceIdVarName,
	...
	public List<BPELPlan> buildScalingPlans(String csarName, AbstractDefinitions definitions, QName serviceTemplateId) {
		...
		List<ScalingPlanDefinition> scalingPlanDefinitions = this
		...
		for (ScalingPlanDefinition scalingPlanDefinition : scalingPlanDefinitions) {
			...
			AbstractPlan abstractScaleOutPlan = this.generateSOG(new QName(processNamespace, processName).toString(),
					definitions, serviceTemplate, scalingPlanDefinition);

			this.printGraph(abstractScaleOutPlan);

			BPELPlan bpelScaleOutProcess = this.planHandler.createEmptyBPELPlan(processNamespace, processName,
			...
			for (AnnotatedAbstractNodeTemplate stratNodeTemplate : scalingPlanDefinition.selectionStrategy2BorderNodes) {
				...
				if (selectionPlugin != null) {
					BPELScopeActivity scope = this.planHandler.getTemplateBuildPlanById(stratNodeTemplate.getId(),
							bpelScaleOutProcess);
					selectionPlugin.handle(new BPELPlanContext(scope, propMap, serviceTemplate), stratNodeTemplate,
							new ArrayList<String>(stratNodeTemplate.getAnnotations()));
	...
	public BPELPlanContext createContext(AbstractNodeTemplate nodeTemplate, BPELPlan plan, PropertyMap map) {
		return new BPELPlanContext(this.planHandler.getTemplateBuildPlanById(nodeTemplate.getId(), plan), map,
				plan.getServiceTemplate());
	}

	public BPELPlanContext createContext(AbstractRelationshipTemplate relationshipTemplate, BPELPlan plan,
			PropertyMap map) {
		return new BPELPlanContext(this.planHandler.getTemplateBuildPlanById(relationshipTemplate.getId(), plan),
				map, plan.getServiceTemplate());
	}
	...
	private List<AbstractNodeTemplate> fetchNodeTemplates(AbstractTopologyTemplate topology,
	...
	private List<AbstractRelationshipTemplate> fetchRelationshipTemplates(AbstractTopologyTemplate topology,
	...
	private List<ScalingPlanDefinition> fetchScalingPlansDefinitions(AbstractTopologyTemplate topology,
		...
		for (String scalingPlanName : scalingPlanNamesRaw) {
			...
			if (!scalingPlanName.trim().isEmpty() && tags.containsKey(scalingPlanName.trim())) {
				...
				List<AbstractNodeTemplate> nodeTemplates = this.fetchNodeTemplates(topology,
				...
				List<AbstractRelationshipTemplate> relationshipTemplates = this.fetchRelationshipTemplates(topology,
				...
				List<AnnotatedAbstractNodeTemplate> selectionStrategy2BorderNodes = this
	...
	private List<AnnotatedAbstractNodeTemplate> fetchSelectionStrategy2BorderNodes(
			AbstractTopologyTemplate topologyTemplate, String selectionStrategyBorderNodesCSV) {
		...
		Map<String, String> selectionStrategyBorderNodesMap = this
		...
		for (String selectionStrategy : selectionStrategyBorderNodesMap.keySet()) {
			AbstractNodeTemplate node = this.fetchNodeTemplate(topologyTemplate,
	...
	private AnnotatedAbstractNodeTemplate findAnnotNode(List<AnnotatedAbstractNodeTemplate> annotNodes,
			AbstractNodeTemplate node) {
	...
	private AbstractRelationshipTemplate getFirstInfrastructureRelation(AbstractNodeTemplate nodeTemplate) {
		List<AbstractRelationshipTemplate> relations = ModelUtils.getOutgoingRelations(nodeTemplate,
	...
	private void runProvisioningLogicGeneration(BPELPlan plan, AbstractNodeTemplate nodeTemplate, PropertyMap map) {
		...
		BPELPlanContext context = new BPELPlanContext(
	...
	private void runProvisioningLogicGeneration(BPELPlan plan, AbstractRelationshipTemplate relationshipTemplate,
			PropertyMap map) {
		// handling relationshiptemplate

		BPELPlanContext context = this.createContext(relationshipTemplate, plan, map);
	...
	private void runProvisioningLogicGeneration(BPELPlan plan, PropertyMap map,
