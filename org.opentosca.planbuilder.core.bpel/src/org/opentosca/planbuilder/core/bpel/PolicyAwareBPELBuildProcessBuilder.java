...
public class PolicyAwareBPELBuildProcessBuilder extends AbstractBuildPlanBuilder {
	 ...
	 */
	private boolean runPlugins(BPELPlan buildPlan, PropertyMap map) {
		for (BPELScopeActivity templatePlan : buildPlan.getTemplateBuildPlans()) {
			...
			if (templatePlan.getNodeTemplate() != null) {
				...
				BPELPlanContext context = new BPELPlanContext(templatePlan, map, buildPlan.getServiceTemplate());
				...
				if (nodeTemplate.getPolicies().isEmpty()) {
					...
					IPlanBuilderPolicyAwareTypePlugin policyPlugin = this.findPolicyAwareTypePlugin(nodeTemplate);
					if (policyPlugin == null) {
						...
						if (chain == null) {
							...
							List<AbstractPolicy> policies = nodeTemplate.getPolicies();
							Map<AbstractPolicy, IPlanBuilderPolicyAwarePrePhasePlugin<BPELPlanContext>> compatiblePrePlugins = new HashMap<AbstractPolicy, IPlanBuilderPolicyAwarePrePhasePlugin<BPELPlanContext>>();
							Map<AbstractPolicy, IPlanBuilderPolicyAwarePostPhasePlugin<BPELPlanContext>> compatiblePostPlugins = new HashMap<AbstractPolicy, IPlanBuilderPolicyAwarePostPhasePlugin<BPELPlanContext>>();

							for (AbstractPolicy policy : policies) {
								boolean matched = false;
								for (IPlanBuilderPolicyAwarePrePhasePlugin<?> policyPrePhasePlugin : this.pluginRegistry
								...
								for (IPlanBuilderPolicyAwarePostPhasePlugin<?> policyPostPhasePlugin : this.pluginRegistry
							...
							if (policies.size() != compatiblePrePlugins.keySet().size()
								...
								for (AbstractPolicy policy : compatiblePrePlugins.keySet()) {
									compatiblePrePlugins.get(policy).handle(context, nodeTemplate, policy);
								}

								for (AbstractPolicy policy : compatiblePostPlugins.keySet()) {
