...
public class BPELPlanContext implements PlanContext {
	 ...
	 * @param serviceTemplateName the name of the ServiceTemplate where the Template
	 *                            of the context originates
	 * @param map                 a PropertyMap containing mappings for all Template
	 *                            properties of the TopologyTemplate the
	 *                            ServiceTemplate has
	 */
	 ...
	 * TopologyTemplate
	 *
	 * @param variable a property variable (var must belong to a topology template
	 *                 property) to check
	 ...
	 * @param myRole                the name of a role inside the partnerLinkType
	 *                              for the myRole
	 * @param partnerRole           the name of a role inside ther partnerLinkType
	 *                              for the partnerRole
	 ...
	 * @throws WSDLException is thrown when either the given File is not a WSDL File
	 *                       or initializing the WSDL Factory failed
	 */
	 ...
	 * @param param2propertyMapping If a Map of Parameter to Variable is given this
	 *                              will be used for the operation call
	...
	public boolean executeOperation(final AbstractRelationshipTemplate relationshipTemplate, final String interfaceName,
		...
		final OperationChain chain = BPELScopeBuilder.createOperationCall(relationshipTemplate, interfaceName,
				operationName);
		if (chain == null) {
			return false;
		}
	...
	public PropertyVariable getPropertyVariable(final AbstractNodeTemplate nodeTemplate, final String propertyName) {		
		for(PropertyVariable propertyVariable : this.propertyMap.getNodePropertyVariables(this.serviceTemplate, nodeTemplate)) {
	...
	public PropertyVariable getPropertyVariable(final AbstractRelationshipTemplate relationshipTemplate,
		...
		for(PropertyVariable var : this.propertyMap.getRelationPropertyVariables(this.serviceTemplate, relationshipTemplate)) {
	 ...
	 * @param directionSink whether to look in direction of the sinks or sources (If
	 *                      Template is NodeTemplate) or to search on the
	 *                      Source-/Target-Interface (if template is
	 ...
	 * @throws WSDLException is thrown when the WSDLFactory to read the WSDL can't
	 *                       be initialized
	 */
	...
	public String getVariableNameOfProperty(final AbstractRelationshipTemplate templateId, final String propertyName) {
		...
		for(PropertyVariable propVar : this.propertyMap.getRelationPropertyVariables(this.serviceTemplate, templateId)) {
