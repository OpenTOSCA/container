...
public class EmptyPropertyToInputInitializer {
	...
	/**
	 * Adds an element to the plan input with the given namen and assign at runtime
	 * the value to the given variable
	 *
	 * @param buildPlan
	 *            the plan to add the logic to
	 * @param propLocalName
	 *            the name of the element added to the input
	 * @param var
	 *            the variable to assign the value to
	 * @param context
	 *            a context for the manipulation
	 */
	private void addToPlanInput(BPELPlan buildPlan, String propLocalName, Variable var, BPELPlanContext context) {
		// add to input
		context.addStringValueToPlanRequest(propLocalName);

		// add copy from input local element to property
		// variable
		String bpelCopy = this.generateCopyFromInputToVariableAsString(this.createLocalNameXpathQuery(propLocalName),
				this.createBPELVariableXpathQuery(var.getName()));
		try {
			Node bpelCopyNode = ModelUtils.string2dom(bpelCopy);
	 ...
	 *
	 * @param inputQuery
	 *            the query to a local element inside the input message
	 * @param variableQuery
	 *            the query to set the value for
	...
	public void initializeEmptyPropertiesAsInputParam(List<BPELScopeActivity> bpelActivities, BPELPlan plan,
			PropertyMap propMap) {
