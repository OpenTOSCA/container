...
public class BPELPolicyAwareBuildProcessBuilder extends AbstractBuildPlanBuilder {
	...
	public BPELPlan buildPlan(final String csarName, final AbstractDefinitions definitions,
		...
		final Property2VariableMapping propMap = this.propertyInitializer.initializePropertiesAsVariables(newBuildPlan,serviceTemplate);
		// init output
		...
		String serviceInstanceURLVarName = this.serviceInstanceHandler
				.findServiceInstanceUrlVariableName(newBuildPlan);
		this.serviceInstanceHandler.appendSetServiceInstanceState(newBuildPlan,
				newBuildPlan.getBpelMainFlowElement(), "CREATING", serviceInstanceURLVarName);
		this.serviceInstanceHandler.appendSetServiceInstanceState(newBuildPlan,
				newBuildPlan.getBpelMainSequenceOutputAssignElement(), "CREATED", serviceInstanceURLVarName);
	...
	public List<AbstractPlan> buildPlans(final String csarName, final AbstractDefinitions definitions) {
		...
		for (final AbstractServiceTemplate serviceTemplate : definitions.getServiceTemplates()) {
			...
			if (!serviceTemplate.hasBuildPlan()) {
				BPELPolicyAwareBuildProcessBuilder.LOG.debug(
						"ServiceTemplate {} has no BuildPlan, generating BuildPlan", serviceTemplate.toString());
				final BPELPlan newBuildPlan = buildPlan(csarName, definitions, serviceTemplate);

				if (newBuildPlan != null) {
					BPELPolicyAwareBuildProcessBuilder.LOG
		...
		for (final BPELScope templatePlan : buildPlan.getTemplateBuildPlans()) {
			...
			if (templatePlan.getNodeTemplate() != null) {
				...
				if (nodeTemplate.getPolicies().isEmpty()) {
					...
					final IPlanBuilderPolicyAwareTypePlugin policyPlugin = this.pluginRegistry
							.findPolicyAwareTypePluginForCreation(nodeTemplate);
					if (policyPlugin == null) {
						...
						if (chain == null) {
							BPELPolicyAwareBuildProcessBuilder.LOG.warn(
						...
						BPELPolicyAwareBuildProcessBuilder.LOG.info(
				...
				if (canGenericPluginHandle(relationshipTemplate)) {
					...
					BPELPolicyAwareBuildProcessBuilder.LOG.debug(
							"Couldn't handle RelationshipTemplate {} with type plugin", relationshipTemplate.getId());
				}
