...
public class Exporter extends AbstractExporter {
	 ...
	 *
	 * @param destination
	 *            the absolute location to export to
	 * @param buildPlan
	 *            the BuildPlan to export
	 * @throws IOException
	 *             is thrown when reading/writing to the given URI fails
	...
	public File export(List<AbstractPlan> plans, CSARID csarId) {
		...
		List<BPMN4ToscaPlan> bpmn4ToscaPlans = new ArrayList<>();

		for (AbstractPlan plan : plans) {
			if (plan instanceof BPELPlan) {
				bpelPlans.add((BPELPlan) plan);
			} else if (plan instanceof BPMN4ToscaPlan) {

				bpmn4ToscaPlans.add((BPMN4ToscaPlan) plan);
			}
		}

		return this.exportBPEL(bpelPlans, bpmn4ToscaPlans, csarId);
	}

	public File exportBPEL(List<BPELPlan> plans, List<BPMN4ToscaPlan> bpmn4ToscaPlans, CSARID csarId) {
		...
		try {
			...
			for (BPMN4ToscaPlan plan : bpmn4ToscaPlans) {
				File destination = new File(tempDir, generateRelativeBPMN4ToscaPath(csarName));
				this.simpleExporter.export(destination, plan);
			}
	 ...
	 *
	 * @param buildPlan
	 *            the BuildPlan to get the path for
	...
	private String generateRelativeBPMN4ToscaPath(String csarName) {
		final String extension = ".bpmn4tosca.json";
		final String prefix = "Plans/";
		String result = prefix + "BPMN4Tosca-" + System.currentTimeMillis() + extension;
		if (csarName != null) {
			if (csarName.endsWith(".csar")) {
				result = prefix + csarName.replace(".csar", extension);
			} else {
				result = prefix + csarName + extension;
			}
		}
		return result;
	}
