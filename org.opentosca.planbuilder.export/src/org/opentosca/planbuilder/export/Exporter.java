...
public class Exporter extends AbstractExporter {
	...
	private TPlan generateTPlanElement(final BPELPlan generatedPlan) {
		...
		final PlanType planType = generatedPlan.getType();
		final String planURL = planType.getString();

		plan.setPlanType(planURL);

		final String id = generatedPlan.getBpelProcessElement().getAttribute("name");
		plan.setId(id);
