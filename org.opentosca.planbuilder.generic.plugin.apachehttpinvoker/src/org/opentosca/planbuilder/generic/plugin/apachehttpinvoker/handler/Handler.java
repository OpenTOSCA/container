package org.opentosca.planbuilder.generic.plugin.apachehttpinvoker.handler;
...
public class Handler {
	...
	public boolean handle(TemplatePlanContext templateContext) {
		...
		for (AbstractNodeTemplate nodeTemplate : templateContext.getNodeTemplates()) {
			if (org.opentosca.planbuilder.generic.plugin.apachehttpinvoker.Plugin.isUbuntuNodeTypeCompatible(nodeTemplate.getType().getId())) {
		...
		Variable httpdPackageVar = templateContext.createGlobalStringVariable(httpdPackageVarName, "apache2");
