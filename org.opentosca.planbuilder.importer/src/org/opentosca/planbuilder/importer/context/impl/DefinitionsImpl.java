...
public class DefinitionsImpl extends AbstractDefinitions {
  ...
  private List<AbstractFile> resolveImportedDefinitions() {
    ...
    if (this.definitions.getImport() != null) {
      for (final TImport imported : this.definitions.getImport()) {
        DefinitionsImpl.LOG.debug(
            "Check import element with namespace: {} location: {} importType: {}",
  ...
  private void initTypesAndTemplates() {
    for (final TExtensibleElements element : this.definitions
      ...
      if (element instanceof TNodeTypeImplementation) {
        this.addNodeTypeImplementation(
            new NodeTypeImplementationImpl((TNodeTypeImplementation) element, this));
      }
   ...
   */
  public void addNodeTypeImplementation(
      final NodeTypeImplementationImpl nodeTypeImplementationImpl) {
   ...
   *        DefinitionsImpl
   */
  public void addRelationshipTypeImplementation(
      final RelationshipTypeImplementationImpl relationshipTypeImpl) {
  ...
  private Definitions parseDefinitionsFile(final AbstractFile file) {
    ...
    try {
      final JAXBContext jaxbContext = JAXBContext.newInstance(
          "org.oasis_open.docs.tosca.ns._2011._12", ObjectFactory.class.getClassLoader());
      final Unmarshaller unmarshaller = jaxbContext.createUnmarshaller();
      DefinitionsImpl.LOG.debug("Trying to parse file {} into JAXB object", file.getPath());
      def =
          (Definitions) unmarshaller.unmarshal(new InputStreamReader(file.getFileAsInputStream()));
    } catch (final JAXBException e) {
      DefinitionsImpl.LOG
          .error("Error while parsing file, maybe file is not a TOSCA Defintions File", e);
      return null;
