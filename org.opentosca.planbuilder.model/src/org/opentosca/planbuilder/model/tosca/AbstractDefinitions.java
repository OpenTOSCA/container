...
public abstract class AbstractDefinitions {
	...
	public AbstractRelationshipType getRelationshipType(QName relationshipTypeId) {
		...
		List<AbstractDefinitions> defs = this.getImportedDefinitions();
		for (AbstractDefinitions def : defs) {
			if (def.getRelationshipType(relationshipTypeId) != null) {
				return def.getRelationshipType(relationshipTypeId);
			}
		}
		return null;
	}
	...
	public AbstractNodeType getNodeType(QName nodeTypeId) {
		List<AbstractNodeType> nodeTypes = this.getNodeTypes();
		for (AbstractNodeType nodeType : nodeTypes) {
		...
		List<AbstractDefinitions> defs = this.getImportedDefinitions();
		for (AbstractDefinitions def : defs) {
			if (def.getNodeType(nodeTypeId) != null) {
				return def.getNodeType(nodeTypeId);
			}
		}
		return null;
	}
