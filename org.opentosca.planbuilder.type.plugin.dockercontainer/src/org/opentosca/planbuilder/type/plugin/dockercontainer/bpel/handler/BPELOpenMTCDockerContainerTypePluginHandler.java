...
public class BPELOpenMTCDockerContainerTypePluginHandler
	...
	public boolean handleOpenMTCGateway(BPELPlanContext templateContext, AbstractNodeTemplate backendNodeTemplate) {
		...
		try {
			...
			String envVarXpathQuery = "concat('ONEM2M_CSE_ID=',$" + tenantIdVar.getName() + ",'~',$"
					...
					+ ",';LOGGING_LEVEL=INFO;ONEM2M_REGISTRATION_DISABLED=false;ONEM2M_NOTIFICATION_DISABLED=false;ONEM2M_SP_ID=smartorchestra.de;ONEM2M_REMOTE_CSE_ID=smartorchestra.de')";
		...
		if ((containerImageVar == null) || BPELPlanContext.isVariableValueEmpty(containerImageVar, templateContext)) {
			...
			final AbstractDeploymentArtifact da = DockerContainerTypePlugin.fetchFirstDockerContainerDA(nodeTemplate);
	...
	public boolean handleOpenMTCProtocolAdapter(BPELPlanContext templateContext, AbstractNodeTemplate openMtcGateway,
			AbstractNodeTemplate sensorNodeTemplate) {
		if (templateContext.getNodeTemplate() == null) {
			LOG.warn("Appending logic to relationshipTemplate plan is not possible by this plugin");
			return false;
		}
		...
		try {
			...
			/*
			 * -e "EP=http://gateway:8000" \ -e "LOGGING_LEVEL=INFO" \ -e "DEVICES=[]" \ -e
			 * 'DEVICE_MAPPINGS={
			...
			String envVarConcatXpathQuery = "concat('EP=http://',$" + gatewayContainerIpVar.getName() + ",':',$"
					...
					+ ",';LOGGING_LEVEL=INFO;DEVICES=[];DEVICE_MAPPINGS={\"',$" + sensorDeviceId.getName()
					+ ",'_1\": \"Hof\", \"',$" + sensorDeviceId.getName() + ",'_2\": \"Hof\"};SIM=false')";
