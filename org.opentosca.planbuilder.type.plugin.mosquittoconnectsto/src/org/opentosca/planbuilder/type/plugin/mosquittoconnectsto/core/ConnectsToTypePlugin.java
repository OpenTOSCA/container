...
public abstract class ConnectsToTypePlugin<T extends PlanContext> implements IPlanBuilderTypePlugin<T> {
	...
	for (AbstractRelationshipTemplate relation : relationshipTemplate.getTarget().getOutgoingRelations()) {
	    ...
	    if (ModelUtils.getRelationshipBaseType(relation).equals(ModelUtils.TOSCABASETYPE_HOSTEDON) && relation
		    .getTarget().getType().getId().equals(ConnectsToTypePluginConstants.MOSQUITTO_NODETYPE)) {
