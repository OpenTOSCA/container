...
public class SimplePlanBuilderBPMNServiceInstanceHandler {
    ...
    public void appendCreateServiceInstanceVarsAndInitializeWithInstanceDataAPI(BPMNPlan bpmnPlan) {
        BPMNDataObject dataObject = new BPMNDataObject(BPMNComponentType.DATA_OBJECT_ST, ServiceInstanceURLVarKeyword + bpmnPlan.getInternalCounterId());
        String idPrefix = BPMNComponentType.CREATE_ST_INSTANCE.name();
        String idSubprocessPrefix = BPMNComponentType.SUBPROCESS.toString();
        BPMNSubprocess createServiceInstanceSubprocess = new BPMNSubprocess(BPMNComponentType.SUBPROCESS, idSubprocessPrefix + "_" + dataObject.getId());
        BPMNSubprocess createServiceInstanceTask = new BPMNSubprocess(BPMNComponentType.CREATE_ST_INSTANCE, idPrefix + "_" + bpmnPlan.getIdForNamesAndIncrement());
    ...
    public BPMNSubprocess addServiceInstanceHandlingFromInput(final BPMNPlan bpmnPlan) {
        String idSubprocessPrefix = BPMNComponentType.SUBPROCESS.toString();
        BPMNSubprocess dataObjectSubprocess = new BPMNSubprocess(BPMNComponentType.SUBPROCESS, idSubprocessPrefix + "_InputOutput_DataObject" + bpmnPlan.getIdForNamesAndIncrement());
        BPMNDataObject dataObjectInput = new BPMNDataObject(BPMNComponentType.DATA_OBJECT_INOUT, "DataObject_InputOutput");
    ...
    private String getCorrespondingServiceInstanceVariable(BPMNPlan plan, String serviceInstanceIDVarKeyword) {
        ...
        for (BPMNDataObject dataObject : plan.getDataObjectsList()) {
            ...
            if (dataObject.getDataObjectType() == BPMNComponentType.DATA_OBJECT_ST || dataObject.getDataObjectType() == BPMNComponentType.DATA_OBJECT_INOUT) {
    ...
    public void addServiceInstanceSetState(String state, BPMNPlan bpmnPlan) {
        BPMNSubprocess setInstanceState = new BPMNSubprocess(BPMNComponentType.SET_ST_STATE, "Activity_ServiceInstanceState" + bpmnPlan.getIdForOuterFlowTestAndIncrement());
