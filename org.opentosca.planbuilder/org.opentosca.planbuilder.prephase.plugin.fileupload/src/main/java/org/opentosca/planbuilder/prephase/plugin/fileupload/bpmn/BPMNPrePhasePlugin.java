...
public class BPMNPrePhasePlugin implements IPlanBuilderBPMNPrePhasePlugin<BPMNPlanContext>,
    ...
    private final BPMNPrePhasePluginHandler handler = new BPMNPrePhasePluginHandler();
     ...
     */
    @Override
    public boolean handle(final BPMNPlanContext context, final TImplementationArtifact ia,
                          final TNodeTemplate nodeTemplate) {
    ...
    public boolean canHandle(BPMNPlanContext context, final TDeploymentArtifact deploymentArtifact,
        ...
        for (final QName artType : ModelUtils.getArtifactTypeHierarchy(ModelUtils.findArtifactTemplate(deploymentArtifact.getArtifactRef(), context.getCsar()), context.getCsar())) {
            for (final QName nodeType : ModelUtils.getNodeTypeHierarchy(infrastructureNodeType, context.getCsar())) {
                BPMNPrePhasePlugin.LOG.debug("Checking if type: " + artType.toString()
    ...
    private boolean isSupportedDeploymentPair(final QName artifactType, final QName infrastructureNodeType,
        ...
        if (!isDA
            && (BPMNPrePhasePlugin.warArtifactType.equals(artifactType) || BPMNPrePhasePlugin.warArtifactTypeOld.equals(artifactType))
        ...
        // debian,..)
        return ModelUtilities.isOfType(BPMNPrePhasePlugin.jarArtifactType, artifactType, artifactTypes)
            || ModelUtilities.isOfType(BPMNPrePhasePlugin.archiveArtifactType, artifactType, artifactTypes)
            || ModelUtilities.isOfType(BPMNPrePhasePlugin.scriptArtifactType, artifactType, artifactTypes)
            || ModelUtilities.isOfType(BPMNPrePhasePlugin.ansibleArtifactType, artifactType, artifactTypes)
            || ModelUtilities.isOfType(BPMNPrePhasePlugin.chefArtifactType, artifactType, artifactTypes)
            || ModelUtilities.isOfType(BPMNPrePhasePlugin.bpelArchiveArtifactType, artifactType, artifactTypes)
            || ModelUtilities.isOfType(BPMNPrePhasePlugin.warArtifactType, artifactType, artifactTypes)
            || ModelUtilities.isOfType(BPMNPrePhasePlugin.warArtifactTypeOld, artifactType, artifactTypes)
            || ModelUtilities.isOfType(BPMNPrePhasePlugin.sqlArtifactType, artifactType, artifactTypes)
            || ModelUtilities.isOfType(BPMNPrePhasePlugin.configurationArtifactType, artifactType, artifactTypes)
            || ModelUtilities.isOfType(BPMNPrePhasePlugin.dockerContainerArtefactTypeOld, artifactType, artifactTypes)
            || ModelUtilities.isOfType(BPMNPrePhasePlugin.dockerContainerArtefactType, artifactType, artifactTypes)
            || ModelUtilities.isOfType(BPMNPrePhasePlugin.tdlConfigurationArtifactType, artifactType, artifactTypes)
            // We always support state artifacts.
            || ModelUtilities.isOfType(BPMNPrePhasePlugin.stateArtifactType, artifactType, artifactTypes);
    }
