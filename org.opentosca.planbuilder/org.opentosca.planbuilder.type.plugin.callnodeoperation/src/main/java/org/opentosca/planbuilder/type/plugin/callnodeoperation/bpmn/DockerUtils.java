...
/**
 * provides methods to interact with NodeTemplate with NodeType Container
 * reference: org.opentosca.planbuilder/org.opentosca.planbuilder.type.plugin.dockercontainer/
 * src/main/java/org/opentosca/planbuilder/type/plugin/dockercontainer/core/DockerUtils.java
 */
public class DockerUtils {
    ...
    private static boolean canHandleDockerContainerPropertiesAndDA(final TNodeTemplate nodeTemplate, boolean ignoreType, Csar csar) {
        ...
        if (!propMap.containsKey("ImageID")
            && EnginePatternHandler.fetchFirstDockerContainerDA(nodeTemplate, csar) == null) {
        ...
        return EnginePatternHandler.isConnectedToDockerEngineNode(nodeTemplate, csar);
    }
    ...
    public static boolean notIsDockerContainer(TNodeType nodeType, Csar csar) {
        ...
        return typeHierarchy.stream().noneMatch(type -> (
            type.getNamespaceURI().equals(DockerContainerConstants.DOCKER_CONTAINER_NODETYPE.getNamespaceURI())
                || type.getNamespaceURI().equals(DockerContainerConstants.DOCKER_CONTAINER_NODETYPE2.getNamespaceURI()))
            &&
            VersionUtils
                .getNameWithoutVersion(type.getLocalPart())
                .equals(DockerContainerConstants.DOCKER_CONTAINER_NODETYPE.getLocalPart())
        );
    }
}
