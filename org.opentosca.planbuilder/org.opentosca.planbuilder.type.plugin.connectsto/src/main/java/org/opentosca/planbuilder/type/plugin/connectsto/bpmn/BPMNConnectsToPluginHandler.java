...
import org.opentosca.planbuilder.type.plugin.connectsto.core.handler.ConnectsToBPMNPluginHandler;
 ...
 */
public class BPMNConnectsToPluginHandler implements ConnectsToBPMNPluginHandler<BPMNPlanContext> {

    private final static Logger LOG = LoggerFactory.getLogger(BPMNConnectsToPluginHandler.class);
    private final static BPMNInvokerPlugin invoker = new BPMNInvokerPlugin();

    @Override
    public boolean handle(final BPMNPlanContext templateContext) {
     ...
     */
    private boolean executeConnectsTo(final BPMNPlanContext templateContext, final TNodeTemplate connectToNode,
        ...
        for (final TInterface iface : ModelUtils.findNodeType(connectToNode, templateContext.getCsar()).getInterfaces()) {
            for (final TOperation op : iface.getOperations()) {
                if (op.getName().equals(Interfaces.OPENTOSCA_DECLARATIVE_INTERFACE_CONNECT_CONNECTTO)) {
                    ...
                    BPMNConnectsToPluginHandler.LOG.debug("Found connectTo operation. Searching for matching parameters in the properties.");
                    ...
                    if (param2propertyMapping.size() != op.getInputParameters().size()
                        ...
                        BPMNConnectsToPluginHandler.LOG.warn("Didn't find necessary matchings from parameter to property. Can't initialize connectsTo relationship.");
                    } else {
        ...
        if (connectsToOp == null) {
            BPMNConnectsToPluginHandler.LOG.warn("No executable connectTo operation found.");
            return false;
        }

        // execute the connectTo operation with the found parameters
        BPMNConnectsToPluginHandler.LOG.debug("Adding connectTo operation execution to build plan.");
     ...
     */
    private boolean findInputParameter(final BPMNPlanContext templateContext,
     ...
     */
    private Map<TParameter, Variable> findInputParameters(final BPMNPlanContext templateContext,
    ...
    private boolean searchBasedOnDefinedOnSource(final boolean definedOnSource, final BPMNPlanContext templateContext,
    ...
    private boolean searchAndAddIfFound(final BPMNPlanContext templateContext,
                                        final String paramName,
                                        final TParameter param,
