...
public abstract class AbstractBPMN4TOSCAPlanBuilder extends AbstractSimplePlanBuilder {
	...
	protected static AbstractPlan generatePlan(final String id, final AbstractDefinitions definitions,
		...
		generateActivitiesAndLinks(activities, links, nodeMapping, topology.getNodeTemplates(), relationMapping,
	...
	private static void generateActivitiesAndLinks(final Collection<AbstractActivity> activities, final Set<Link> links,
		...
		for (final AbstractNodeTemplate nodeTemplate : nodeTemplates) {
			final AbstractActivity activity = new NodeTemplateActivity(nodeTemplate.getId() + "_bpmn4tosca_activity",
					ActivityType.BPMN4TOSCA, nodeTemplate);
		...
		for (final AbstractRelationshipTemplate relationshipTemplate : relationshipTemplates) {
			final AbstractActivity activity = new RelationshipTemplateActivity(
					relationshipTemplate.getId() + "_bpmn4tosca_activity", ActivityType.BPMN4TOSCA,
