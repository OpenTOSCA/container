...
public abstract class AbstractBPMN4TOSCAPlanBuilder extends AbstractSimplePlanBuilder {
    ...
    protected static AbstractPlan generatePlan(final String id, final AbstractDefinitions definitions,
        ...
        if (newInstance) {
            return new AbstractPlan(id, AbstractPlan.PlanType.BUILD, definitions, serviceTemplate, activities,
                links) {};
        } else {
            return new AbstractPlan(id, AbstractPlan.PlanType.MANAGE, definitions, serviceTemplate, activities,
                links) {};
        }
    }
    ...
    private static void generateActivitiesAndLinks(final Collection<AbstractActivity> activities, final Set<Link> links,
        ...
        BPMN4TOSCATemplate activityNode =
            bpmnWorkflow.stream().filter(bpmnActivity -> "StartEvent".equals(bpmnActivity.getName())).findAny()
                        .orElse(null);
        for (int i = 0; i < bpmnWorkflow.size(); i++) {
            for (int connection = 0; connection < activityNode.getConnection().length; connection++) {
                final String nextNodeId = activityNode.getConnection()[connection];
