package org.opentosca.siengine.api.soaphttp.processor;
...
import org.opentosca.siengine.api.soaphttp.model.Doc;
import org.opentosca.siengine.api.soaphttp.model.InvokeOperationAsync;
import org.opentosca.siengine.api.soaphttp.model.InvokeOperationSync;
import org.opentosca.siengine.api.soaphttp.model.InvokePlan;
import org.opentosca.siengine.api.soaphttp.model.ParamsMap;
import org.opentosca.siengine.api.soaphttp.model.ParamsMapItemType;
...
/**
 * Request-Processor of the SIEngine-SOAP/HTTP-API.<br>
 * <br>
 * 
 * Copyright 2013 IAAS University of Stuttgart <br>
 * <br>
 * 
 * This processor processes the incoming requests of the SIEngine-SOAP/HTTP-API.
 * It transforms the incoming unmarshalled SOAP message into a from the SIEngine
 * understandable camel exchange message. The SIHeader-Enum is used here to
...
public class RequestProcessor implements Processor {
	...
	public void process(Exchange exchange) throws Exception {
		...
		if (exchange.getIn().getBody() instanceof InvokeOperationAsync) {
			...
			exchange.getIn().setHeader(SIHeader.NODEINSTANCEID_STRING.toString(), nodeInstanceID);
			...
			exchange.getIn().setHeader(SIHeader.SERVICETEMPLATEID_QNAME.toString(), serviceTemplateID);
			
			String nodeTemplateID = invokeIaRequest.getNodeTemplateID();
			exchange.getIn().setHeader(SIHeader.NODETEMPLATEID_STRING.toString(), nodeTemplateID);
			
			String relationshipTemplateID = invokeIaRequest.getRelationshipTemplateID();
			exchange.getIn().setHeader(SIHeader.RELATIONSHIPTEMPLATEID_STRING.toString(), relationshipTemplateID);
			
			interfaceName = invokeIaRequest.getInterfaceName();
			
			if ((interfaceName != null) && !(interfaceName.equals("?") || interfaceName.isEmpty())) {
				exchange.getIn().setHeader(SIHeader.INTERFACENAME_STRING.toString(), interfaceName);
			}
		...
		if (exchange.getIn().getBody() instanceof InvokePlan) {
			...
			exchange.getIn().setHeader(SIHeader.PLANID_QNAME.toString(), planID);
		...
		if (serviceInstanceID != null) {
			...
			exchange.getIn().setHeader(SIHeader.SERVICEINSTANCEID_URI.toString(), serviceInstanceURI);
		}
		
		exchange.getIn().setHeader(SIHeader.CSARID.toString(), csarID);
		exchange.getIn().setHeader(SIHeader.OPERATIONNAME_STRING.toString(), operationName);
