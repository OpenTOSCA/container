...
public class ExportedInterfacesConsolidation {
	...
	public boolean consolidate(CSARID csarID) {
		...
		for (QName serviceTemplateID : toscaReferenceMapper.getExportedInterfacesOfCSAR(csarID).keySet()) {
			...
			for (TExportedInterface iface : toscaReferenceMapper.getExportedInterfacesOfCSAR(csarID).get(serviceTemplateID)) {
				...
				for (TExportedOperation operation : iface.getOperation()) {
					...
					if (null != planReference) {
						...
						toscaReferenceMapper.storePlanInputMessageID(csarID, planID, getInputMessageQName(csarID, iface.getName(), operation.getName(), toscaPlan.getId(), toscaPlan, toscaReferenceMapper.getListOfWSDLForCSAR(csarID)));
	 ...
	 */
	private QName getInputMessageQName(CSARID csarID, String wsdlInterfaceName, String wsdlOperationName, String planID, TPlan boundaryPlan, List<Document> list) {
		
		LOG.debug("Try to find the InputMessageID for CSAR " + csarID + " and plan " + boundaryPlan.getId());
		
		LOG.debug("countwsdl: " + list.size() + " interfacename:" + wsdlInterfaceName + " operationname:" + wsdlOperationName);
			...
			try {
				...
				if (messageQName.getLength() == 1) {
					...
					if (messages.getLength() == 1) {
						...
						QName messageID = new QName(doc.lookupNamespaceURI(value.substring(0, value.indexOf(":"))), value.substring(value.indexOf(":") + 1));
